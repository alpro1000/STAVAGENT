...
> srovnání (viz §CN níže). Snapshot z `buildLaborProjection`, 2026-06-12.
> Provenance Nh: **[normy potvrzené Alexander, 2026-06 + CN SAFE implied]**
> — potvrzené normy jsou DATA se zdrojem (`labor-norms.ts`), dny VŽDY ze
> scheduleru (normy nikdy nehýbou harmonogramem: 77.5 d / curing 9 /
> prestress 13 beze změny).

| Operace | dní | Nh (norma) | h přítomnost | Nh/m³ | norma |
|---|---|---|---|---|---|
| armování (výztuž B500B, 104 t) | 27.9 | 1 872.0 | 2 340 | 2.70 | 18 Nh/t |
| předpětí (lana Y1860, 19.21 t [VV 422373 ÷ 2]) | 13.0 | 672.4 | 840.5 | 0.97 | 35 Nh/t |
| betonáž (12 os. na 1 finiš-front, 2 čerpadla front krmí; 693.35 m³ / 42.5 m³/h finishing-governed = 16.3 h, rotace > 12 h druhou směnou) | 1.6 | 156.6 | 195.7 | 0.23 | crew-model [Caltrans T1.1] |
| skruž + bednění montáž + demontáž (vč. podpěr; kontakt 1 527.6 m² [CN SAFE]) | 65.8 | 4 735.5 | 5 919.4 | 6.83 | 3.1 Nh/m² |
| ošetřování betonu | 9.0 | 36.0 | 45 | 0.05 | 1 os. × 5 h/d |
| **CELKEM** | — | **7 472.5** | **9 340.6** | **10.78** | — |

✅ kalibrace DOKONČENA (2026-06-12): čtyři potvrzené normy aplikovány jako
data se zdrojem (`Monolit-Planner/shared/src/calculators/labor-norms.ts`):
armování 18 Nh/t · předpětí 35 Nh/t lan · skruž+bednění 3.1 Nh/m² KONTAKTNÍ
plochy (ne půdorysné!) · betonáž crew-model. Norm-based větve mají
canon-fallback, když báze chybí (kontaktní plocha / hmotnost lan). CELKEM
v koridoru 8–12 Nh/m³ (golden assertion). Vstupy navíc v §5f:
`formwork_contact_area_m2: 1527.6` + `prestress_strand_mass_kg: 19210`.

✅ betonáž crew-model KOREKCE dle světových referencí (2026-06-12,
provenance **[Caltrans Bridge Deck Construction Manual (Oct 2015) Table 1.1;
bridge deck method statement 40–45 m³/h finishing-governed; potvrzeno
Alexander]**): JEDEN finiš-front, 2 čerpadla ho krmí (trámy + deska) —
čerpadla NEnásobí četу; na stavbě 12 os. (Caltrans T1.1 = 11: foreman 1 +
rake 2 + operátor lišty 1 + finišeři 2 + brum/ošetřování 1 + vibrace 2 +
tesař dozor skruže 1 + příjem domíchávačů 1; + 1 záloha/parťák); strojníci
čerpadel jsou externí (přijíždějí s čerpadly, do Nh nevstupují); tempo
40–45 m³/h je FINISHING-governed, ne kapacita čerpadel → 16.3 h (střed
42.5); rotace > 12 h = druhá směna 12 os., headcount konstantní. Dřívější
interpretace «12/linku × 2 = 24 os., tandem 30–40» nadhodnocovala betonáž
2.4× (380.4 Nh). Fáze-check: betonáž-fáze 1.6 d × 10 h = 16 h ≈ modelová
zálivka 16.3 h — fáze drží, harmonogram 77.5 / 9 / 13 beze změny.

✅ ošetřování VYŘEŠENO (2026-06-11, rozhodnutí Alexander): `labor-projection`
počítá dny ošetřování jako **max(span fáze zrání ze scheduleru, curing_days)**
— scheduler-fáze zrání v tact_details má pro PDPS 1 takt komprimovaný span
1.5 d (vnitřní dluh scheduleru, příbuzný 220.5/307.8), zatímco `curing_days`
= 9 (třída 4 @15 °C); ošetřovatel je na stavbě po celou dobu zrání → 9 d ×
1 os. × 5 h × 0.8 = 36 Nh. U multi-takt elementů kalendářní span legitimně
přesahuje curing_days — max() jej zachovává. Golden assertion v
`golden-so202.test.ts` §5f-Nh + hermetic test v `labor-projection.test.ts`.
