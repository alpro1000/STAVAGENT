...
**Status**: 163 files sorted across full komplex (A/B/C/D + shared).
Π.0.0 first pass shipped D + shared (58 files). Π.0.0 Part 2 added
A/B/C uploads from ABMV (71 files). **Drop v2 (2026-05-10) added 34
DWGs** for PROBE 9 prostupy + štroby quantification: 24 to D (3 koord
overlays + 18 per-discipline TZB sections + 3 architectural xrefs from
3 ZIPs) + 10 to shared (1.PP per-discipline TZB + komplex utilities).
...
|--------|----:|----:|----:|----:|----:|--------|
| `shared/` | **43** | 13 | 19 | 10 | 1 | ✅ komplex tables + drawings + TZ + 1.PP TZB sections (drop v2) |
...
| `D/` | **49** | 35 | 14 | 0 | 0 | ✅ all D-specific drawings + drop v2 koord overlays + per-discipline TZB |
| **TOTAL** | **163** | **78** | **74** | **10** | **1** | |
...
**UPDATE 2026-05-10 (drop v2):** D-objekt **9410/9420/9430 DWGs now
PRESENT** in `sources/D/dwg/`. Each DWG also unpacked from a ZIP
containing 6 per-discipline TZB drawings (chl/kan/sil/slb/vod/vzt) +
1 architectural xref (UDL_NNP_D) — total 24 new D files. Enables PROBE
9 prostupy + štroby auto-extraction. See
[`sources/D/INVENTORY.md`](D/INVENTORY.md) drop v2 addendum.

**A/B/C still have 9X10/9X20/9X30 DWG missing** — symmetric with prior
state for D. ZIPs for A/B/C exist in `inputs/_UNSORTED/` (Π.0.0 Part 2
backlog) but not yet sorted; D drop v2 was the priority for PROBE 9.
...
- Drop v2 (PROBE 9 sources): 34 files — `test-data/libuse/outputs/drop_v2_classification_manifest.md`
- Move execution: 129 × `git mv` (Π.0.0) + 24 ZIP-extract + 10 `mv` (drop v2) = 163 sorted, 100 % success
- Symlinks: 163 × relative `../sources/...` for cross-machine portability
- All operations preserve git history where possible (`git log --follow` works for renames)

_Generated by Claude Code Π.0.0 Part 3 sorting execution, 2026-05-06; updated 2026-05-07 for A/B/C; updated 2026-05-10 for drop v2._
