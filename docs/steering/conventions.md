...
## 16. Instrument verification (ratified 2026-07-20)

**Любой артефакт, от которого зависят решения — измерительный прогон, метрика,
корпус, генератор отчёта — проходит независимое ревью + самопроверку ДО первого
использования.** Не после того, как числа кому-то показали.

Канонический прецедент: Stage-0 eval harness (`URS_MATCHER_SERVICE/backend/eval/`)
был написан, прошёл 10-угловое независимое ревью → 15 подтверждённых дефектов
(включая «все метрики — константы» и «первый прогон отравляет второй»), был
починен и получил `selfcheck.mjs` (синтетические строки с известными исходами,
проверяющие ПРИБОР, не систему) — всё до первого замера. Если бы корпус был
собран раньше ревью, флип каталога был бы одобрен по числам, которые ничего не
измеряли.

Минимум для measurement-артефакта:
1. Независимое ревью (multi-angle, verify-пасс) до использования.
2. Самопроверка с известными исходами, запускаемая после любой правки артефакта.
3. Запись в артефакте ВСЕХ осей, определяющих поведение прогона (версии
   каталогов, режимы, отпечаток данных) — два прогона должны быть различимы
   по самим артефактам.
4. Изоляция вместо дисциплины: артефакт сам не может тронуть prod-состояние
   (kill-switch, отдельные файлы), а не «оператор должен не забыть».
...
| 20.07.2026 | 1.1 | +§16 Instrument verification (ratified by Alexander after the Stage-0 harness review). |
