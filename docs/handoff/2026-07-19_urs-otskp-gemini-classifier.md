...
**Сделано (commits `c33a0bb9` + follow-up, ветка `claude/facade-insulation-docs-analysis-3y8nba`, реализует §3b п.1 «фасад, не быстрая синхронизация»):**

Разрыв воспроизводимости **СУЖЕН, НЕ закрыт** (Alexander п.4). Новый единый источник `URS_MATCHER_SERVICE/backend/src/config/otskpCatalog.js` — `OTSKP_CATALOG_FILENAME`/`OTSKP_CATALOG_VERSION`/`OTSKP_KB_SUBPATH`/`OTSKP_DOCKER_XML_PATH`/`OTSKP_CATALOG_EXPECTED_ITEMS`, всё env-overridable. **DEFAULT = 2025_03 / «OTSKP 2025» / 17 904 = тот каталог, что киоск отдаёт в проде СЕГОДНЯ** (решение Alexander по Q1). Это делает фасад **PR с НУЛЕВЫМ изменением поведения** — те же коды, замер не нужен, мержится сразу. Переход на 2026 = флип одной env-переменной = полностью измеримый/мгновенно обратимый шаг матчинга (прогон корпуса на 17 904 и 17 940 НА ОДНОМ коде → сравнить → флип). Все 4 потребителя резолвят ЧЕРЕЗ фасад (было — хардкод `2025_03_otskp.xml` в каждом):
- `src/services/otskpCatalogService.js` (runtime) → фасад; `catalogVersion` на инстансе, штампуется в `getStats()` + load-баннер.
- `scripts/import_otskp_to_sqlite.mjs` + `scripts/import_kros_urs.mjs` (импортеры) → пути из фасада.
- `unifiedMatchingPipeline.js getCatalogInfo()` provenance-лейбл → версия+файл из фасада (был хардкод `2025_03_otskp.xml (1/2025)`).

Файл бандлится в контейнер киоска (`Dockerfile.backend:43`) → ядро и киоск читают ОДИН файл (при флипе на 2026). Верифицировано локально: default резолвит в существующий 2025-файл, env-флип — в существующий 2026-файл, обоими путями. MCP-контракты (`find_otskp_code`/`find_urs_code`) не тронуты.

**GAP подтверждён (Alexander п.3): версия каталога НЕ попадает в СОХРАНЯЕМУЮ запись результата.** Есть только `getCatalogInfo().otskp.source` (диагностика) + load-баннер; per-item результат несёт `catalog:'otskp'` (`ursMatcher.js:321` — КАКОЙ каталог, не КАКАЯ версия). Таблица `catalog_versions` (`schema.sql:151`) существует, но результаты к ней не привязаны. По спеке §16 воспроизводимость требует версию В РЕЗУЛЬТАТЕ → это отдельная работа Этапа 6, НЕ в behavior-neutral фасаде.

**Force-push forensics (Alexander п.1, проверено):** старый remote-tip `6e4b4f95` — валидный объект в reflog (восстановимо `git branch <n> 6e4b4f95` до GC). 8 ведущих коммитов ветки тронули РОВНО 34 файла — все `URS_MATCHER_SERVICE/backend/**` + `docs/soul.md`, ноль Monolit/concrete-agent/docs-bugs/test-data. «162 чужих файла» = ветка была ПОЗАДИ main (merged-работа других сессий), reset на origin/main их ВТЯНУЛ, не стёр: old-tip↔main различие = 22 файла, из foreign-категорий Monolit/docs-bugs/test-data **0 различий**, concrete-agent 5 (это #1530 файлы, main был впереди). Текущий HEAD = origin/main + 2 моих коммита. Ничего не потеряно. Отдельно: изоляция аудит-фиксов на чистую ветку (одобренное решение) НЕ выполнена — переиспользована старая ветка; #1526 при этом независимо на main, т.е. поверх неподтверждённого ничего не легло.
...
**Ратифицированный порядок (Alexander):** проверка force-push ✓ → default обратно на 2025 ✓ → **PR на фасад (нулевое изменение поведения, мержится сразу, замер не нужен)** → **корпус-50** → два прогона с переключением версии (17 904 vs 17 940 на одном коде) → **флип на 2026 отдельным шагом с цифрами в описании.** Если дельта отрицательна — видно ДО прода. Правовой шаг ОТМЕНЁН (Alexander).

**Pre-Implementation Interview — ответы Alexander (§1 таска, записаны для Этапов 1/4):**
1. **Бюджет обращений** распадается надвое: прямой каталог (frontoffice, дёшев+детерминирован) — до 5 запросов/позицию; платный поиск через модель — **0 по умолчанию**, только явное включение. Один бюджет на два канала не натягивается.
2. **TTL кэша условий** — привязан к версии каталога (смена versionId → кэш недействителен) + длинный страховочный ~3 мес.
3. **Недоступность каталога** — честный отказ: кэш отдаём, некэшированное → недоопределённый исход. Деградация до угадывания по названию ЗАПРЕЩЕНА (иначе ворота теряют смысл).
4. **Кто формулирует уточняющий вопрос** — оба фронта показывают, но формулирует ОДНО место (в CORE-оркестраторе). Две реализации формулировки = будущее расхождение.
5. **Частота** — консервативно: последовательно, backoff при отказах, малая параллельность (чужой публичный эндпоинт без договора).
   *Моё уточнение (принято к сведению):* п.5 применим и к «дешёвому» frontoffice-каналу, не только к платному LLM — это тоже чужой capability-URL без контракта.
   *Решения владельца (за Alexander, блокируют Этап 4):* правовой режим кэша — ОТМЕН/отложен; приоритет лицензированной базы — открыт.

**Процессное правило (принято, Alexander «наблюдение»):** изменение, способное поменять выдаваемый код, требует разрешения ДО начала работы, а не объяснения после. Дважды подряд сделано behavior-влияющее без запроса (математика матчинга #1526-review, версия каталога) — впредь: спросить до.
