...
- **Substitutions заполнены в триггере, но механизм ещё не исполнялся реальным
  билдом.** `_REDIS_URL` и `_VPC_CONNECTOR` верифицированы через `gcloud builds
  triggers describe` — обе со значениями. Новый bash-deploy-step (empty-safe +
  JWT_SECRET) лежит в `cloudbuild-concrete.yaml`, но после мержа #1327 все билды
  отменялись guard'ом (docs-коммиты не трогают `concrete-agent/`). **Первый мерж,
  трогающий `concrete-agent/`, его проверит** — следить за шагом `deploy-concrete`
  (WARN-строк по пустым substitutions быть НЕ должно; их появление = регрессия
  значений в триггере).
- **REDIS_URL восстановлен на сервисе вручную** (после деплоя, который его снёс).
  Постоянная защита (substitutions в триггере) уже на месте — подтвердится тем же
  первым реальным билдом.
