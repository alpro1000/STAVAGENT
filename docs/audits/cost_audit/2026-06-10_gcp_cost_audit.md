...
- **Объём билдов — ✅ ПОДТВЕРЖДЁН west3-листингом (gcloud 2026-06-10), кратно выше
  оценки по git-log:** видимая выборка June 1 07:00–June 3 17:01 (2.6 дня) =
  **42 пуш-события × 5 триггеров = 210 билдов** (June 1: 20 событий, June 2: 14,
  June 3: 8) → темп **~80 билдов/день ≈ 2 400/мес ≈est** против ранней оценки
  ~290/мес (git-log author-dates — плохой прокси для push-частоты; build-история
  авторитетна). Бо́льшая часть событий = **все 5 FAILURE** (docs-only пуши).
  В burst'ы (5 пушей за 12 мин, June 2 13:02–13:14) часть билдов уходит в
  **EXPIRED** (очередь/квота).
- **Арифметика статьи ЗАКРЫТА:** ~2 400 билдов/мес × ~80 % FAILURE × 1–3 мин
  guard-степа ≈ 1 900–5 800 мин/мес на E2_HIGHCPU_8 — это и есть биллинговые
  ~5 500 мин (1 980 Kč). **Статья Cloud Build ≈ в основном guard-FAILURE-минуты.**
...
- **🔴 Триггер `c5efd974` перманентно красный**: ни одного SUCCESS за всю видимую
  выборку, включая события, где остальные 4 зелёные (June 1 18:59, June 2
  13:39/13:42) — ломается не на guard'е, а по-настоящему. Опознать:
  `gcloud builds triggers list --region=europe-west3 --format='table(id,name)'`
  + лог одного билда. Чинить или выключить — кандидат в задачи.
- Manual submits этим месяцем не драйвер: global-пул за 30 дней содержит один
  submit (2026-06-10, concrete, 5м08с). Реальный concrete-билд с layer-кэшем =
  **~5 мин** ≈ $0.08/билд.
...
| 2b | **Починить/выключить перманентно красный триггер `c5efd974`** (0 SUCCESS за всю выборку) | входит в №1/№2 | диагностика по логу | ~0 |
