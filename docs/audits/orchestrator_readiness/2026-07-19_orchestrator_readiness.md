...
**Кросс-канальное побайтовое сравнение (UI-путь vs MCP-путь, тот же вход) — ВЫПОЛНЕНО.**
Метод: `engine.js:47-105` показывает, что бэкенд `/api/calculate` = `res.json(planElement(req.body))` **verbatim** (тело запроса ЕСТЬ `PlannerInput`, без маппинга); делегат возвращает тело verbatim (`monolit_delegate.py:202`); MCP-тул добавляет только `output["source"]="monolit_planner_api"` (`calculator.py:889`). Значит UI-путь ≡ `planElement(payload)`, MCP-путь ≡ то же + `source`. Собрал shared (`tsc`), прогнал ровно тот `PlannerInput`, что строит `_build_planner_payload` для моего входа, через локальный `planElement`, и сверил с живым MCP.
- **UI-путь run-to-run:** 2 прогона **побайтово идентичны** — `cmp` без разниц, `sha256 = 6d4053b5…`, 15 854 Б оба раза.
- **UI-путь vs MCP-путь:** сверил **42 leaf-поля** по всем секциям (element/pour/formwork/rebar/schedule/resources/props/bridge_technology/costs/deadline/resource_ceiling) — **все 42 совпали точно**, включая `costs.total_labor_czk = 962385.19` (до копейки), `schedule.total_days = 90.5`, `pour.tact_volume_m3 = 115.56`, `rebar.mass_kg = 17334`, `bridge_technology.mss_cost.total_czk = 11910000`, `props.total_props_needed = 148`, `deadline.fastest_total_cost_czk = 1292581`.
- **Единственная структурная разница каналов:** добавленное тулом поле `source:"monolit_planner_api"` — `ui_expected_mcp.json` (UI + source) = 15 886 Б (= 15 854 + это поле).
- **Result-id / timestamp:** отсутствуют в ОБОИХ каналах (в UI-выходе `has("source")=false`, единственное добавленное MCP-поле — `source`; никакого id/timestamp).

**Вывод:** UI-путь и MCP-путь дают **побайтово тот же результат на том же входе, с точностью до добавляемого тулом поля `source`** — оба зовут идентичный `planElement` (`engine.js:105` verbatim → `monolit_delegate.py:202` verbatim → `calculator.py:889` +source). Оговорка: сверял против кода этой ветки, собранного локально; живой прод Cloud Run воспроизвёл те же числа в §3.7 выше (byte-identical ×2), т.е. прод задеплоен с этого же движка.
...
- **AC4** — детерминизм с фактическими числами двух прогонов: §3.7 (MCP ×2 побайтово + локальный golden ×2 + кросс-канальное побайтовое UI↔MCP: 42/42 поля, sha256 UI = 6d4053b5…). ✅
