...
**Co dál:** довести `claude/urs-frontoffice-catalog-search` до зелёного CI + merge; после деплоя — `GET /api/batch/diagnostics` → секция `frontoffice` должна быть `OK, candidatesFound≥1`; опционально metadata-авторезолв + Gemini→Vertex.
