...
**Otevřené otázky:** CI (mcp-compat) na push — lokální fastmcp blocker trvá; MCP-transport (FastMCP dispatch) není stage-gated vůbec (opt-in session model přes REST session_id — stejné jako před Gate 5, ne regrese); zbytek UWO out-of-scope fronty (F2 port sandbox S2–S10, keyword-kalibrace, DE/ES sloty).

**Co dál:** report Alexandrovi (Gates 4+5 hotové, SPEC §9.1 odblokován) + čekat CI; potom dle fronty.
