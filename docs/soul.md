...
## 2026-07-24 — Session: сквозная оркестрация + детерминизм прогона + реальный счёт OTSKP (17 940)
**Rozhodnuto:** (1) **Аудит готовности к оркестратору** (`docs/audits/orchestrator_readiness/2026-07-19_*`, ветка `claude/formwork-architecture-audit-r98c31`, PR не открывался по заданию) — 12 тем, 7 критериев приёмки закрыты. Главные выводы: тяжёлый расчёт НЕ дублирован (делегирование в TS-движок, SSOT), но **классификация дублирована алгоритмически** (данные едины через codegen, скоринг реализован дважды с разными порогами ambiguity и разными confidence: TS near-tie ≤3 + градуированная формула vs Python exact-tie + плоские 0.9/0.7); **оркестрации на MCP-поверхности нет**; 5 независимых кэшей «замороженных фактов», staleness-guard только у одного (soupis-handles, и тот — ручной инкремент `PARSE_VERSION`, не контент-хэш); правило маршрутизации каталогов захардкожено, `kb/urs_otskp_routing.yaml` кодом не читается, ветка `design_build` схлопнута в `urs`; BC3/GAEB отсутствуют. AC4 закрыт живыми числами: два прогона MCP байт-идентичны + два локальных golden-прогона + **кросс-канальная сверка UI↔MCP 42/42 поля** (`total_labor_czk` 962385.19 до копейки; единственная структурная разница — добавляемое тулом поле `source`; result-id нет ни в одном канале). (2) **Закрыты две худшие находки аудита** (PR #1552): новый MCP tool **`run_document_to_soupis`** (23→24) — structure → plan → decompose → export в ФИКСИРОВАННОМ порядке (порядок = код, не промпт), элементы для декомпозиции берутся из `plan.mapping.elements` (те же элементы, что движок оценил — нет второго, независимо выведенного списка); **детерминизм** `app/services/pipeline_run.py` — `run_id` контентно адресован (sha256 канонизованных входов + pipeline_version + catalog_version), пер-стадийные sha256 входа/выхода, `stages_sha256` = отпечаток всей цепочки, канонический JSON сортирует ключи; пропущенная стадия ВСЕГДА в манифесте с причиной, упавшая останавливает цепочку и репортится verbatim. (3) **OTSKP 17 904 → 17 940**: 2026 SFDI base был уже живой (DB из него отвечала), но литерал `17 904` жил в ~33 живых плоскостях — описания MCP-тулов сообщали агентам устаревший размер каталога. Посчитано из коммитнутого XML (streamed iterparse, уникальные `<znacka>`): `2025_03` 17 904 vs `2026_otskp.xml` **17 940**. Правились ТОЛЬКО живые плоскости; датированные исторические артефакты не тронуты. Новый `tests/test_otskp_catalog_count.py` выводит счёт из XML и требует согласия каждой цитирующей плоскости.
**Odmítnuto:** переписывание датированных исторических доков (`docs/archive`, `docs/audits`, `docs/handoff`, CHANGELOGs, session-логи, замороженные пилотные `test-data/**`) — каждый корректно фиксирует величину на свою дату; «починка» указателя `2026_otskp.xml` в `otskp.py` (я ошибочно принял разницу имён МЕЖДУ ВЕТКАМИ за баг — на main указатель верен, правка откачена); добавление оркестратора в стадию workflow YAML (он стадии ВЕДЁТ, а не является одной из них).
**Otevřené otázky:** дубль алгоритма классификации (TS vs Python пороги/confidence) — сводить или считать допустимой двухрантаймовостью (смежно с SO-250 classify-delegation, решение за Alexander); эфемерность passport/calculator_facts на Cloud Run при scale-out; `PARSE_VERSION` как ручной guard; `kb/urs_otskp_routing.yaml` vs захардкоженный `_primary_catalog` — какой источник правды; детерминированный Bind-адаптер (ADR-009 D2) не построен; учёт реального расхода токенов LLM (сейчас оценка `len//4`, sink отсутствует; измеримы только бизнес-кредиты).
**Co dál:** живая проверка `run_document_to_soupis` на реальном SO после деплоя (манифест + повтор run_id на том же входе); затем — по решению Alexander — либо сведение классификаторов, либо Bind-адаптер.

## 2026-07-23 — Session (g): Этап 1 построен и ИЗМЕРЕН — STOP-условие сработало (fabrication на ÚRS-корпусе выросла), доложено, PR не открыт
**Rozhodnuto:** (1) **Этап 1 (SPEC §3) реализован полностью** по ратификации «go etap 1», ветка `claude/facade-insulation-docs-analysis-3y8nba`, БЕЗ PR (см. STOP ниже): (а) `foldDiacritics` + фолдинг внутри `normalizeText` (единственный источник нормализации двери; числа/классы сохраняются — доказано тестами «tř. 3»/«DN 100»/«tl. 100 mm»/C25/30); (б) колонка `urs_items.search_name` (= normalizeText(name+' '+description), пишут ОБА импортера, ALTER-if-missing) → SQL-отбор фолд-симметричен по построению, legacy-fallback через COALESCE; (в) `src/services/intentExtractor.js` — детерминированная структура намерения {action, object, material_specs[], dimensions[], unit(ТОЛЬКО из výměry), context, supply_scope(только явные маркеры)}, каждое поле {value, source, confidence}, отсутствующее = null; (г) интеграция в `matchUrsItemsLocal`: phrase-pass (спеки/размеры отдельным селективным запросом LIMIT 200, гарантированное включение) + прежний word-pass по search_name; (д) 17 новых тестов (287/287 зелёные), включая DB round-trip обе стороны фолдинга. (2) **Замер до/после** (до = stash на код базлайна, ТЕ ЖЕ корпуса/DB-файлы; после = пересобранные eval-DB с search_name; selfcheck 32/32 оба раза): **OTSKP 2026 (прод-дефолт): 1.0/1.0 → 1.0/1.0, регрессий НОЛЬ** ✅. **OTSKP 2025: top1/recall 0.9231 → 0.9615** (+3.8 п.п.: 272324/272365 теперь верны и на 2025-данных) НО per-line регресс otskp-013 (было верно 327365 → стало 327222, expected выпал из списка). **ÚRS: recall 0 → 0** (данными: 35/41 кодов физически нет в 2018-базе), **nodiacritics-механизм ДОКАЗАН: все 5 пар «без диакритики ↔ с диакритикой» дают ИДЕНТИЧНЫЕ списки кандидатов** (было 0 кандидатов), но **fabrication 0.8333 → 1.0 (ХУЖЕ), honest_skip 0.1667 → 0 (ХУЖЕ)**. (3) **Диагноз обеих деградаций — НЕ фолдинг**: (а) ÚRS-fabrication: улучшенный отбор теперь находит кандидатов и на nonexistent-строках (conf 0.25–0.42, одна 0.71) — отсутствие floor честного отказа (известный класс, тикет «go floor») УСИЛЕНО лучшим retrieval; (б) otskp-013: лучший локальный fuzzy-хит conf 0.712 перешагнул существовавший гейт `bestLocalConf < 0.7` → OTSKP-supplement (который держал верный ответ 0.9) перестал вызываться — архитектурная бородавка гейта, вскрытая фиксом. (4) **STOP-правило исполнено**: «ухудшение любой метрики = остановиться и доложить» — работа закоммичена в ветку, PR НЕ открыт, доклад с вариантами у Alexander'а.
**Odmítnuto:** самовольное добавление floor в этот же PR (floor = отдельная ратификация «go floor», меняет выдаваемые коды); самовольная правка гейта 0.7 (тот же класс — меняет выдачу, нужно разрешение ДО).
**Otevřené otázky:** (за Alexander) вариант squашивания: Этап 1 + floor одним измеренным пакетом (floor убивает conf 0.25–0.42 фабрикации; одна строка 0.71 переживёт floor 0.5–0.6) vs Этап 1 ждёт floor; гейт supplement 0.7 (otskp-013): supplement всегда + сортировка по conf, или порог 0.75, или как есть; eval/README поправлен (2026 теперь дефолт + предупреждение «пересобери eval-DB после изменения нормализации»).
**Co dál:** слово Alexander'а → потом PR Этапа 1 (с floor или без), после него frontoffice-query-стратегия §5.2 из полей намерения (интеграционная точка готова: intent.search_phrases/action/object).

## 2026-07-23 — Session (g, продолжение): очередь floor → Этап 1 → гейт исполнена, ЧЕТЫРЁХТОЧЕЧНАЯ СЕРИЯ ЗАКРЫТА
**Rozhodnuto:** Alexander ратифицировал: (1) вариант (б) — floor ОТДЕЛЬНЫМ PR ПЕРВЫМ (пара «floor 0 → floor+Э1 0.1667» = прецедент «каждое улучшение retrieval поднимает fabrication без floor»); (2) гейт 0.7 → supplement всегда + минимальное правило «каталожная природа при равном/большем скоре выше fuzzy» (без перекалибровки шкал — Этап 3); порог floor НЕ подгонять под корпус (0.71-остаток честно стоит до ворот Этапа 2); (3) README-caveat 2018-оси. Исполнено тремя PR по очереди, каждый со своим замером: **#1540 floor** (fuzzy-шкала only, порог = существующая константа MEDIUM 0.6, valve URS_LOCAL_CONF_FLOOR; ÚRS fabrication 0.8333→0.0, honest_skip→1.0, все 6 nonexistent честные; OTSKP-оси бит-в-бит; инертность пересобранных eval-DB доказана предварительно бит-в-бит) → **#1541 Этап 1** (cherry-pick 7d1739c2 поверх floor; +Migration 3 в db/init.js: ALTER search_name + BACKFILL 39K строк на старте — строгие floor-тесты вскрыли мину: без колонки дверь молча умирала SQL-ошибкой, а COALESCE-фолбэк на конце-акцентных словах («potrubí» vs «potrubi») ХУЖЕ старого коверканья; снисходительные легаси-тесты мину не видели) → **#1542 гейт** (supplement всегда, det-over-fuzzy tie-break в merge; хирургия: единственная изменённая строка на трёх осях = otskp-013 исправлена). **Серия (база → floor → floor+Э1 → gate):** OTSKP 2026 (прод) 1.0/1.0 на ВСЕХ точках · OTSKP 2025 top1/recall 0.9231 → 0.9231 → 0.9615 → **1.0/1.0** · ÚRS fabrication 0.8333 → **0** → 0.1667 → 0.1667 (остаток = urs-044 conf 0.708, пре-адъюдикирован) · honest_skip 0.1667 → **1.0** → 0.8333 → 0.8333 · nodiacritics twin-пары 5/5 идентичны с floor и после гейта. Suite 273→290, selfcheck 32/32 на каждом шаге.
**Odmítnuto:** floor+Э1 одним PR (стёрло бы прецедент); порог 0.75 (подгонка); полная калибровка шкал сейчас (Этап 3).
**Otevřené otázky:** повторный СЕТЕВОЙ базлайн по README (Alexander, Cloud Shell) — теперь на четырёхточечной базе; OTSKP-ось fabrication 1.0 (otskp-023, supplement-шкала conf 0.76) = задокументированный остаток до ворот Этапа 2; известный остаток urs-044.
**Co dál:** сетевой базлайн → frontoffice-query-стратегия §5.2 в find_urs_code из полей намерения (3 условия ратифицированы ранее) → карантин 2018-двери → Этап 2 (ворота/баллы/исходы).

## 2026-07-23 — Session (g, дополнение): класс «промах на различителе способа производства» (174111101 ručně vs 174151101 strojně)
**Rozhodnuto:** Alexander проверил в живом KROS: `174151101` СУЩЕСТВУЕТ — это strojně-версия, а запрос был ručně; ответ модели «Text zadání přesně odpovídá názvu» — ложь (в названии strojně). Класс опаснее фабрикации: реальный код, одна цифра в середине, правдоподобное имя, проходит глазами; проверка существования его не ловит; в метриках прячется в общем top1-промахе. Директива исполнена: (1) **`intentExtractor` + 2 поля-модификатора** — `execution_method` (ručně/strojně, склонённые формы conf 0.8, «strojovna»/«ručník» не матчатся, противоречие в строке → null, отсутствие → null и НИКОГДА не читается как значение) и `compaction` (se zhutněním / bez zhutnění / vč. zhutnění); (2) **корпус: пара urs-048/049** (174111101/174151101, идентичный попис, различие только в способе, категория spec — единственная пара этого класса); (3) **Этап 2: способ производства = ЖЁСТКИЕ ВОРОТА, не баллы** (кандидат с противоречащим способом отсекается независимо от текстового сходства) — записано как ратифицированное требование в докблоке экстрактора. Замер: существующие 47 ÚRS-строк бит-в-бит (поля инертны для отбора), пара честно возвращает пусто (вся семья 174 отсутствует в 2018-базе — задокументированная мёртвая зона; пара выстрелит на живом канале, где ошибка и родилась). Suite 297/297. Попутно: v2-заметка чекера уточнена — příplatky несут СВОЮ каталожную MJ по семье (174111109 за m³, клемпíř за kus): валидировать против каталожной единицы кода, не против «kus» напевно.
**Otevřené otázky:** ворота Этапа 2 (способ+zhutnění) — реализация при ратификации Этапа 2; пара 048/049 на живом канале — при сетевом базлайне.
**Co dál:** без изменений (сетевой базлайн → frontoffice-стратегия → карантин 2018 → Этап 2).

## 2026-07-23 — Session (g, вечер): карантин 2018 поднят в очереди и исполнен; Perplexity ≠ поиск по сайту (зафиксировано)
**Rozhodnuto:** Alexander развёл две вещи, которые нельзя путать: (1) **2018-каталог — полное исключение из выдачи за флагом, дефолт выключен** (не понижение веса); (2) **Perplexity — НЕ поиск по сайту ÚRS**: живой curl показал на «ručně» ответ «strojně» conf 0.72 с обоснованием «text přesně odpovídá» при собственном признании «bez přímého zobrazení stránky» — модель пересказывает каталог по памяти; правдоподобный пересказ опаснее мёртвого кода (мёртвый видно сразу, подменённый способ уходит в смету). Порядок: карантин → frontoffice-стратегия коротких запросов (иначе ÚRS-канал останется на одном Perplexity — хуже) → Perplexity последним источником с маркером, никогда top-1 при живом каталожном кандидате. **PR A (карантин) исполнен**: `db/catalogVintage.js` (`vintageQuarantineSql()`, фильтр `source='kros'` ≡ импорт URS201801; valve `URS_ALLOW_2018_CATALOG=1`) применён ко ВСЕМ дверям, предлагающим коды (ursMatcher local+tech-rules, ursLocalMatcher jobs-двери, knowledgeBase keyword search); admin/browse/stats витрины СОЗНАТЕЛЬНО не фильтруются (витрина не должна лгать о содержимом базы). Migration 4 (`source` колонка ALTER-if-missing + schema.sql truth) — та же мина, что search_name: SQL по колонке, которой нет в базе из schema.sql → дверь молча умирает; вскрыто полным прогоном (floor-тесты), в изоляции 3/3 зелёные. Замер: **ÚRS fabrication 0.1667 → 0, honest_skip → 1.0** (пре-адъюдицированный остаток urs-044 умер ВМЕСТЕ с карантином — его код и был предложением из 2018-данных), 2 изменённые строки (обе → честное пусто), recall 0→0; **OTSKP оба оси бит-в-бит**. Suite 300/300. Зафиксирован вывод Alexander'а: **каталог решает проблему покрытия, не проблему выбора** — доступ к полному каталогу не лечит ни один из измеренных дефектов (174151101 = обе позиции настоящие; 7 дверей мимо frontoffice = маршрутизация; fabrication 0.83 была при живом OTSKP под рукой; больше кандидатов без ворот = ВЫШЕ fabrication, уже наблюдали 0→0.17); при подключении полного каталога — прогнать тот же корпус до/после подключения (самое ценное измерение: доля данных vs доля логики).
**Otevřené otázky:** PR B (frontoffice короткие запросы из полей намерения) — следующий, свой замер; Perplexity-канал CORE find_urs_code — отдельная очередь (3 условия).
**Co dál:** PR B → сетевой базлайн (Cloud Shell, 49 строк) → find_urs_code → Этап 2.

## 2026-07-23 — Session (g, ночь): PR B — frontoffice-стратегия коротких запросов; ТРЕТИЙ случай «молчание ≠ успех» — в самом счётчике
**Rozhodnuto:** (1) **PR B реализован**: `buildFrontofficeQuery(text, intent)` — короткий запрос из полей намерения (action + object + **execution_method** сырыми токенами С диакритикой — сервер каталога владеет чешским, фолдинг только наша внутренняя конвенция); длинные строки (>5 значимых токенов) идут коротким запросом, короткие/голые коды нетронуты (null → legacy full-text); возвращённая семья сиблингов ре-ранжируется по similarity к ПОЛНОМУ описанию внутри конфиденс-яруса (exact 1.0 / variant 0.9 / text-hit 0.8|0.75 сохранены). Герметичный E2E-тест на мотивирующем классе: ручně-строка → top-1 **174111101**, strojně-близнец → **174151101** (различитель решает); отправленный query ≤5 слов и несёт «ручně». (2) **Открытие: egress песочницы на \*.run.app блокирован прокси (CONNECT 403)** — frontoffice отсюда никогда не был достижим; мои локальные «сетевые» прогоны считали proxy-403 как «resolved calls» (spy честно резолвил fetch). Третий случай класса «молчание ≠ успех», на этот раз в измерительном приборе: fetch-spy теперь пишет `online_status` (разбивку HTTP-статусов) в run record — прогон с {'403': 50} больше никогда не прочитается как «канал опрошен». Локальный корпус: ноль дрейфа (бит-в-бит, детерминизм), OTSKP нетронут, suite 307/307, selfcheck 32/32. **Живая дельта PR B = прогон Cloud Shell по README** (Alexander) — теперь на 49 строках с парой 174.
**Otevřené otázky:** живой замер стратегии (Cloud Shell); пара 048/049 должна впервые получить шанс на recall > 0 (живой каталог 2026 не ограничен 2018-данными).
**Co dál:** сетевой базлайн → find_urs_code (3 условия) → Этап 2.

## 2026-07-24 — Session (g, живой прогон): frontoffice-канал ЖИВ но ДРОССЕЛИРУЕТСЯ — online_status поймал; harness запацен
**Rozhodnuto:** Живой Cloud Shell-прогон ÚRS-корпуса (49 строк) вернул `online_status {"200":11,"429":39}` — **третий «молчание ≠ успех» пойман ПРИБОРОМ**: канал достижим (11×200, urs_catalog CS_URS_2026_02), но harness бьёт запросы подряд → лимит ~10 req/min выбран за первые ~11 вызовов, остальные **39/50 получили HTTP 429**. Вывод: метрики (top1 0 / recall 0) НЕЛЬЗЯ читать как результат стратегии — 39 строк дросселированы, не измерены. Вторичный сигнал (осторожно, малая выборка): среди 7 code-bearing строк **ноль frontoffice-яруса (conf ≥ 0.75)** — все OTSKP-supplement дорожные коды 0.32–0.46; значит даже ~10 достигших каталог (200) получили ПУСТО из /v1/search коротким запросом. Честность держится вживую: 6/6 nonexistent → честный отказ (fabrication 0, honest_skip 1.0), пара 174 → пусто (карантин + нет живого ответа). **Harness запацен** (instrument-only, коды не меняет): (1) `--online-delay-ms` (дефолт 6500 в urs, 0 в otskp) — пейсинг между строками, ~5 мин на 49; (2) per-line `online_status` в каждой строке результата — теперь видно построчно 200-пусто vs 429-throttled (агрегат не различал). selfcheck 32/32; run-record несёт `online_delay_ms`. README: caveat rate-limit + «читать online_status ДО метрик».
**Otevřené otázky:** **паценый re-run из Cloud Shell** (≥49 clean 200) = первая ВАЛИДНАЯ дельта стратегии — покажет, короткий запрос реально будит /v1/search или возвращает пусто (тогда нужна другая форма запроса, не «Zásyp sypaninou ručně», а напр. одно слово-объект/автокомплит); пара 174 получит первый шанс на recall.
**Co dál:** паценый сетевой re-run → построчный разбор 200-ответов → (если пусто) итерация формы запроса §5.2 → find_urs_code → Этап 2.

## 2026-07-24 — Session (g, чистый прогон): frontoffice отвечает 200 но ПУСТО на всех 49 → диагностический probe
**Rozhodnuto:** Паценый Cloud Shell re-run (delay 6500): `online_status {"200":50}` — **ноль дросселя, первый ВАЛИДНЫЙ замер**. Вердикт однозначен: **на всех 49 строк живой `/v1/search` вернул 200 и НОЛЬ frontoffice-кандидатов**. Единственные code-bearing строки — те же 7 OTSKP-supplement дорожных кодов 0.32–0.46 (все неверные); ни одного conf≥0.75. top1/recall 0, честность держится (6/6 nonexistent → отказ, пара 174 → пусто). Throttling больше не оправдание — reached-and-empty вживую. Стратегия коротких запросов как построена **не будит** живой endpoint. Две гипотезы под «200+пусто на каждый текст»: (1) **stale versionId** — клиент пинит `dsdCAHQZh6lFvriEi3aB`, при ротации релиза ÚRS всё пусто при 200 (дискриминатор: голый код 174111101 — живой versionId ВСЕГДА находит существующий код); (2) **клиент читает не ту секцию** — URL шлёт textsPage/categoriesPage/itemsPage (фасетный ответ), а `searchCatalog` берёт ТОЛЬКО `advancedSearch.items`; текстовые совпадения могут лежать в `.texts`/`.categories`, которые клиент выбрасывает (голый код мы ни разу не тестировали → `items` не проверен). Матчер спекулятивно НЕ трогаю (403 из песочницы, замерить нельзя) — построен **read-only `eval/probe-frontoffice.mjs`**: голый код + одно слово + фраза + автокомплит + метадата, дампит ВСЕ array-пути ответа (вскрывает секцию, если результат там) + резолвнутый versionId. Хелперы probe проверены на синтетическом фасетном ответе — секцию `.texts` находят.
**Otevřené otázky:** запуск probe из Cloud Shell → выбор гипотезы → фикс (versionId-резолв ИЛИ harvest texts/categories) с авторизацией + паценый замер; OTSKP-supplement шумит дорожными кодами на ÚRS-строках (0.32–0.46) — отдельный вопрос (cross-catalog, is_cross_catalog помечен).
**Co dál:** probe → диагноз → фикс формы запроса/секции → паценый re-run → find_urs_code → Этап 2.

## 2026-07-22 — Session (f): сетевые прогоны Alexander'а — три находки канала + вердикт по смете Vidímova
**Rozhodnuto:** (1) **Сетевой ÚRS-базлайн (полный канал)**: online_attempts 48, все resolved — и метрики БИТ-В-БИТ = офлайн-нулю (top1 0 / recall 0 / fabrication 0.83). Per-line доказательство: все 47 конфиденсов 0.21–0.68 (шкала локальной двери), ни одного 0.75–1.0 (шкала frontoffice) → **`/v1/search` вернул ПУСТО на все 47 длинных verbatim-описаний**, канал тихо провалился на 2018-дверь. Вывод: **frontoffice-фикс только маршрутизацией дал бы НОЛЬ** — нужна query-стратегия §5.2 Level 1 (короткие запросы из полей намерения) = пререквизит **Этап 1** (структура намерения). (2) **nodiacritics 5/5 → пустой список кандидатов** (с диакритикой те же строки дают кандидатов) — локальный SQL-отбор диакритико-чувствителен несмотря на folding #1526 (folding лёг в similarity, не в отбор). (3) **Чекер, прогон 1 (400 мс)**: 149 «не найдено» = АРТЕФАКТ — frontoffice лимитирует ~10 req/min (первые 10 lookup'ов OK, дальше пустые 429-ответы, прочитанные как вердикт — «молчание ≠ успех» в моём же инструменте). Фикс #1537 (смержен): захват HTTP-статуса через fetchImpl, статус `OMEZENO` + retry 65s, default delay 6500 мс; проверено стабом (429/429→OMEZENO; 429/200-пусто→NENALEZEN; 200-hit→OK). (4) **Чекер, прогон 2 (чистый)**: OMEZENO=0; **все 129 K-работ получили ответ** (108 OK: sim медиана 1.0; 21 флаг), **все 38 M-материалов не найдены (38/38)** → frontoffice-поиск индексирует PRÁCE, материалы-спецификации вне покрытия — статус `MATERIAL_NELZE_OVERIT` добавлен в чекер. **Вердикт по смете Vidímova**: ядро здоровое; реальные кандидаты на правку — r484/r488 (коды PŘÍPLATKŮ 764215646/764216665 использованы как основные oplechování-работы!), r471 (762395000: каталог m3, смета soubor), r234 (621211011: podlahy vs каталожные PODHLEDY), r486/r490 (RŠ-банды: 385→код rš330, 170→rš330), r498 (svod DN150 vs каталожный DN100?), r292/r301 (MJ m2den vs m2 — семантика верна, этикетка нет).
**Поправка вердикта (Alexander, подтверждена данными):** r484/r488 — НЕ ошибки кодирования: оба příplatku стоят непосредственно под родительскими oplechování-позициями (764215608→764215646, 764216644→764216665, общий 6-значный префикс) — код верен, остаётся оформление (MJ `1 soubor` прячет базу доплаты; чистая форма kus × počet rohů + явная привязка к родителю). **Системное требование к чекеру v2 (записано в header скрипта): příplatek-позиции проверяются ПАРОЙ, не строкой** — каталожное имя «Příplatek…» → искать соседа с тем же 6-значным префиксом в том же dílu, валидировать пару (родитель есть? MJ kus? počet vs геометрия родителя?), флаг только при отсутствии пары — иначе ложный поплах на каждом корректном příplatku. r471 (m³ vs soubor) остаётся в силе — вопрос размерности цены, не контекста.
**Otevřené otázky:** ратификация Этапа 1 (за Alexander) — на него теперь завязаны и frontoffice-query-стратегия, и диакритика-фикс; floor честного отказа (отдельная маленькая правка, go за Alexander); остальные вердикты по флагам сметы (r471, r234 podlahy-vs-podhledy, RŠ-банды) — за Alexander; материалы: верификация требует другого источника (SPCM), вне текущего канала.
**Co dál:** слово Alexander'а по Этапу 1 + floor → потом frontoffice-фикс уже с query-стратегией, measured.

## 2026-07-22 — Session (e): OTSKP-корпус SO-250 принят + ДЕЛЬТА 2025→2026 ИЗМЕРЕНА (вопрос «что дали 36 позиций» закрыт)
**Rozhodnuto:** (1) PR **#1534 смержен** (`e9a533fa`, CI 3/3) — весь Stage-0 слой в main. (2) **OK Alexander'а по SO-250** → `eval/corpus.otskp.jsonl` закоммичен (27 строк: 10 plain / 12 spec / 4 nodia / 1 nonexistent `9112B1.R`; коды проектанта из тендерного XC4, 23/24 проверены в обоих каталогах). (3) **Дельта снята** (два прогона, ОДИН код, только env+`--db`; selfcheck перед прогоном 32/32): **OTSKP 2025: top1 0.9231 / recall 0.9231 · OTSKP 2026: top1 1.0 / recall 1.0 → +7.7 п.п., регрессий НОЛЬ** (STOP-правило: ни одной отрицательной дельты). Per-line доказательство: `272324` ZÁKLADY C25/30 и `272365` VÝZTUŽ ZÁKLADŮ — в 2025 матчер отвечал `317324`/`317365` (ŘÍMSY — не тот элемент!), в 2026 отвечает верно. Fabrication 1.0 → 1.0 (единственная nonexistent-строка сфабрикована в ОБОИХ — no-floor универсален, не ÚRS-специфичен). **Попутно доказано: OTSKP-дверь РАБОТАЕТ (92→100 % top1)** — ÚRS-ноль из session (c) был данными (2018), не алгоритмом. (4) Вывод: **флип фасада на 2026 обоснован цифрами** — improvement без регрессий; шаг предодобрен ратифицированным порядком («флип отдельным шагом с цифрами»), оформляется отдельным PR.
**Otevřené otázky:** fabrication на OTSKP-оси измерена по 1 строке (грубо) — расширить nonexistent из соседних SO пакета D6; сетевой ÚRS-прогон + чекер Vidímova — за Alexander'ом.
**Co dál:** PR корпуса → PR флипа 2026 (цифры в описании) → floor честного отказа (measured) → frontoffice в `find_urs_code` (3 условия) → карантин 2018-двери.

## 2026-07-20 — Session (d): обратный режим — ревизия сданной сметы (чекер кодов + completeness-проверка)
**Rozhodnuto:** Alexander: 2018-каталог «вычеркни и не вспоминай» — только живые 2026 (KROS/ÚRS); проверить КАЧЕСТВО сданной сметы: (а) валидность код↔описание, (б) полнота против документации («ничего ли не забыто»). **(а) Построен `scripts/audit_estimate_codes.mjs`** — обратный режим матчера («код+описание → проверь пару»): парсит KROS Export Komplet, для каждого каталожного кода → живой frontoffice (exact lookup, conf 1.0): существует? название совпадает (ловит коллизии перенумерации)? MJ совпадает? Рабочие коды → честно MIMO_KATALOG без lookup'ов; локальные 2018-данные ВЕДОМО не используются; sequential + delay 400ms (Q5-дисциплина); `--dry` смоук на реальном Vidímova: 203/167/36 — парсер точен. Сетевой прогон за Alexander'ом (egress). **(б) F2-quick прогнана здесь** (детерминированно, R3-контрольник vs сданный KROS): 29 строк со статусом DOPLNIT/ZAHRNOUT/ROZPOR → 18 доехали; **кандидаты на ЗАБЫТО (после точечной верификации fuzzy-ложняков): K07 oplechování styku střechy/fasády (Q15 «doplním» — длина так и не пришла?), APU profily (Q16), dilatační profily ETICS, diagonální výztuhy síťoviny, sanační malta spár, keramický soklík balkonů, ocelový překlad O17 (статус ZAHRNOUT A OCENIT!), přesun hmot klempířské 998764102 (13 строк 764* есть, přesun нет), odvoz lešení (dovoz 993112111 есть, odvoz не виден)**; мягкие: lepicí hmota dlažby (возможно v ceně montáže). Ложняки сняты: O06→набор 7678811xx, HIZ.BAL→771591207, rohové→622143003, dovoz lešení→993112111. Пустых množství в сданном: **0**. Подтверждение методики: Šimek сам в письме спрашивал про profily — проверка воспроизвела его сомнение механически.
**Otevřené otázky:** сетевой прогон чекера (Alexander); F2-full против TZ/чертежей = CORE-пайплайн, нужны PDF документации; кандидаты «забыто» — вердикт за Alexander'ом (часть может быть v ceně komplete-позиций).
**Co dál:** прогнать чекер в сети → вердикт по кандидатам → (продукт-идея: «ревизия сметы» как режим — в backlog при следующем груминге).

## 2026-07-20 — Session (c): корпус Vidímova ПРИНЯТ + ПЕРВЫЕ ЦИФРЫ (offline ÚRS базлайн) + SO-250 OTSKP-кандидаты
**Rozhodnuto:** (1) **Корпус ÚRS принят Alexander'ом**: 47 строк из пары Vidímova (сданный KROS Export = ground truth, направление подтверждено допзаявками Šimka: 67,8 m² spádové klíny → `713141311` и т.д.; моя первая интерпретация «система сгенерировала → человек выбросил» была ИНВЕРСИЕЙ — отозвана). Категория `supply_volume` → **`work_vs_material`** (montáž↔dodávka, разные описания); supply_volume (ОДИНАКОВОЕ описание, разный код по объёму поставки) = **0 строк, в источнике не встретилось, проверка уедет на Этап 4** — не фабриковать. 6 nonexistent подтверждены (сданы заказчику с рабочими кодами O02/O05/O05-REV/Z01/O07/11210R-PROJ-KAC). Закоммичен как `eval/corpus.urs.jsonl`. (2) **ПЕРВЫЕ НАСТОЯЩИЕ ЦИФРЫ КАЧЕСТВА МАТЧЕРА** (offline, локальная дверь, frontoffice выключен и так недостижим, `urs_kros.db` 39 397 строк из committed CSV, URS_LEARNING=0): **top1 = 0.0 · recall = 0.0 · fabrication = 0.83 · honest_skip = 0.17** (n=47, errors 0). (3) **Диагноз recall=0 — ДАННЫЕ, не (только) алгоритм**: локальная ÚRS-дверь собрана из `URS201801.csv` = каталог **2018**, смета = CS ÚRS **2026** → **35/41 ожидаемых кодов в базе ОТСУТСТВУЮТ** (recall структурно ≤15 %); **коллизия перенумерации** — `59054296` = «profil ukončovací s okapničkou» (2026) vs «Dlážděné kryty pozemních komunikací» (2018): тот же код, другой предмет (тихая ошибка при код-матче!); «имена» в базе = огрызки keywords («cm geontx m/b polystrvch») → даже 6 присутствующих кодов recalled 0/6. Fabrication 83 % = отсутствие floor честного отказа (montáž отвечен кодами материалов, conf 0.22–0.68). Тикет: `docs/bugs/urs-local-door-2018-vintage/` (P1). Вывод: frontoffice (живой CS ÚRS 2026) — единственная дверь с правильной датой; полный сетевой прогон Alexander снимет сам по README — его дельта против этого базлайна и есть «что даёт frontoffice» в цифрах. (4) **OTSKP-корпус по директиве**: источник = SO 250 Zárubní zeď из тендерного XC4 (D6 Olšová Vrata–Žalmanov, AspeEsticon, `cenova_soustava=OTSKP 2025` в каждой строке — Aspe штампует версию в запись!), 24 позиции проектанта → **27 кандидатов** (10 plain / 12 spec / 4 nodia / 1 nonexistent=`9112B1.R`), **23/24 кодов проверены в ОБОИХ каталогах** (2025 и 2026) → OTSKP-корпус мерит матчинг, не дыры данных. Кандидаты у Alexander'а на просмотре, НЕ закоммичены. `import_kros_urs.mjs` получил `--db` (зеркало соседнего импортера).
**Odmítnuto:** добивание корпуса до 50 (Alexander: «не добивай»); фабрикация supply_volume строк; трактовка recall=0 как «качество алгоритма» без диагноза данных.
**Otevřené otázky:** OTSKP-кандидаты ждут просмотра (особенно единственный nonexistent — fabrication на 1 строке груба; расширение из соседних SO пакета = follow-up); полный сетевой ÚRS-прогон за Alexander'ом; OTSKP-базлайны 2025/2026 после ОК корпуса.
**Co dál:** ОК OTSKP-корпуса → базлайны OTSKP 2025 vs 2026 (два прогона, один код) → сетевой ÚRS-прогон (Alexander) → frontoffice-фикс за флагом → re-measure → флип 2026 с цифрами.

## 2026-07-20 — Session (b): routing-матрица + xhigh-ревью Stage-0 прибора → 15 дефектов → ремонт + selfcheck
**Rozhodnuto:** (1) **Routing-матрица** (2 независимых трассера, read-only): «3 или 8» = ни то, ни другое — ОДНА структурная причина. Единственная полная дверь = `matchUrsItems` (L4 learned → L2 frontoffice → L5 web/local → L1 OTSKP-supplement); **7 дверей обходят L2** (real ÚRS): `find_urs_code` (сам канонический ÚRS-вход структурно web-only — `urs.py:219` → `/api/pipeline/match` → `unifiedMatchingPipeline` без `searchCatalog`), `create_work_breakdown`(ÚRS), pipeline/match(+batch), `/api/batch`, block-match-fast, Portal urs-match proxy; те же обходят L4; block-match-fast держит ВТОРУЮ расходящуюся память (`kb_mappings`); Portal `/portal/analysis` вообще не матчит (passport verbatim-коды); orchestrator-readiness аудит НИКОГДА не запускался. Артефакт: `docs/audits/routing_matrix/2026-07-20_catalog_matching_routing_audit.md` (все 14 file:line цитат независимо перепроверены sweep-ом). Вердикт: точечный фикс `find_urs_code` = stopgap, лечение = одно ядро (SPEC §2) — но смена архитектуры вне scope таска, решает Alexander. (2) **xhigh code-review Stage-0 прибора ДО первого замера** (10 углов, 4 независимых файндера, verify, sweep): **15 подтверждённых дефектов** — топ: `.code` vs `.urs_code` (ВСЕ метрики константы: top1=0, fabrication=0, honest=1.0 на любом корпусе), stdout-загрязнение (`> results.json` нечитаем, `--compare` падает), auto-learn отравляет второй прогон (дельта структурно ~0), bare `--truncate` рушит общую `urs_items` + OTSKP-коды заучиваются как ÚRS через relabel `source:'local'` В ОБХОД анти-poison гейта #1526, пустой корпус = exit 0 «чистый PASS». (3) **Ремонт по go Alexander (4 условия)**: kill-switch `URS_LEARNING=0` в `matchUrsItems` (inert по умолчанию; прод-дыра гейта = ОТДЕЛЬНЫЙ тикет `docs/bugs/urs-learn-gate-source-relabel/`), изоляция вместо дисциплины (обязательный `--db` per-version eval-DB + floor 1000 против 36-item toy-seed + отпечаток path/count в run record), явный `--mode otskp|urs` (otskp глушит frontoffice), **`selfcheck.mjs` — 32 assertions PASS** (синтетика проверяет ПРИБОР: hit/nonexistent/miss/error/legacy-code + все parser-rejections + compare-guards). Обе catalog-оси + все env-ручки в run record; `--compare` отказывает cross-corpus/cross-mode, UNMEASURABLE-warning вместо null-как-«no change», направления STOP per-metric (fabrication = RISE). unit/quantity в корпусе задокументированы «collected, not measured» (unit-gate = Stage 2). Норма ратифицирована → `conventions.md §16` (instrument verification до использования).
**Odmítnuto:** сбор корпуса до ремонта прибора (числа были бы красивы и пусты); фикс прод-дыры гейта внутри eval-коммита (отдельный measured-шаг после базлайна).
**Otevřené otázky:** curl достижимости URS Matcher из CORE (за Alexander — `matcher:0` может значить «Perplexity-only всё время», swallow на `urs.py:257-259`); источник ground-truth пар корпуса (за Alexander); frontoffice-routing фикс `find_urs_code` (go есть, 3 условия: parallel source / env-флаг / отдельный PR — ПОСЛЕ базлайна ÚRS на текущем канале).
**Живая верификация ремонта (после коммита `0ea639ec`):** jest-сьют киоска **270/270 PASS** (CI на claude/-ветках не бегает — только PR; прогнано локально с `NODE_OPTIONS=--experimental-vm-modules`); полный E2E-смоук через РЕАЛЬНЫЙ пайплайн: обе eval-БД собраны импортером с `--db` (2025=17 904 / 2026 env-flip), оба прогона + `--compare` отработали (run records различимы по осям, guards чисты, exact-name строки: top1=1.0 через local door, 0 online calls). **Живой сигнал системы (не прибора): у матчера НЕТ floor честного отказа** — на бессмысленную строку («kvantové bednění neexistující fantomové práce») вернулся код 27512 с conf 0.31 → fabrication на smoke-строке = 1.0. Один синтетический пример ≠ замер, но направление ясно: реальный корпус, вероятно, покажет высокую fabrication; категория nonexistent обязательна не зря. Смоук-артефакты в scratchpad/gitignored, в репо не попали, базлайном НЕ являются.
**Co dál:** curl (Alexander) → корпус-50 (источник за Alexander) → базлайны OTSKP 2025/2026 + ÚRS current → frontoffice-фикс за флагом → re-measure → флип 2026 с цифрами.

## 2026-07-20 — Session: OTSKP-фасад в URS_MATCHER (reviewer #1 — разрыв воспроизводимости)
**Rozhodnuto:** Alexander: «правовой режим опускаем, важно реализовать технологию» → из пересмотренного порядка рецензии убран правовой шаг, реализован **фасад OTSKP** (reviewer #1, «одна дверь, не быстрая синхронизация копий»). Ветка `claude/facade-insulation-docs-analysis-3y8nba` была **stale** (23 коммита позади `origin/main`, её 8 ahead-коммитов = уже squash-merged #1526 URS-аудит) → сброшена на `origin/main` (Pattern 12 orphan, force-with-lease). На свежей базе: ядро на `2026_otskp.xml` (17 940) через #1530, но URS_MATCHER matching-дверь всё ещё парсила `2025_03_otskp.xml` (17 904) — один запрос через CORE и киоск давал разные коды. **Commit `c33a0bb9` + follow-up:** новый единый источник `URS_MATCHER_SERVICE/backend/src/config/otskpCatalog.js` (`OTSKP_CATALOG_FILENAME`/`OTSKP_CATALOG_VERSION`/…, всё env-overridable), 4 потребителя резолвят через него (было — хардкод в каждом). **DEFAULT = 2025_03/«OTSKP 2025»/17 904 = каталог прода СЕГОДНЯ** (решение Alexander Q1) → фасад = PR с НУЛЕВЫМ изменением поведения, мержится сразу без замера; 2026 = флип одной env-переменной = измеримый/обратимый шаг матчинга. Верифицировано: default резолвит в существующий 2025-файл, env-флип — в 2026, обоими путями. MCP не тронуты.
**Odmítnuto:** (1) default 2026 в самом фасаде — по форме PR смешивал бы рефактор с правкой поведения (та смесь, что уже разделяли); → 2025. (2) правка Portal+Monolit — их живой OTSKP кормится НЕ ручным `import-otskp.js`, а `otskpAutoImport.js`/`migrations.js`/`routes/otskp.js`/Monolit `Dockerfile:20 COPY "…2025_03 OTSKP.xml"` (все 2025, имя с пробелом, Render-era legacy); перевод требует 17 МБ XML в build-context каждого репо (не бандлят KB ядра) — Docker здесь не собрать → откачено, один infra-gated тикет (handoff §5). Правовой шаг ОТМЕНЁН (Alexander).
**Otevřené otázky:** **GAP — версия каталога НЕ в сохраняемой записи результата** (только `getCatalogInfo()` диагностика + баннер; per-item несёт `catalog:'otskp'`, не версию) → §16/Этап 6; корпус-50 не снят (delta 17 904→17 940 не измерен); Portal/Monolit фасад ждёт provisioning XML; изоляция аудит-фиксов на чистую ветку НЕ выполнена (переиспользована старая ветка, но #1526 независимо на main — поверх неподтверждённого ничего не легло). Force-push проверен: ничего не потеряно, anchor `6e4b4f95` в reflog, 8 коммитов = 34 файла URS+soul, «162 чужих» = ветка была позади main.
**Co dál (ратифицированный порядок):** проверка force-push ✓ → default 2025 ✓ → **PR на фасад (мержится сразу)** → **корпус-50** → два прогона (17 904 vs 17 940 на одном коде) → флип на 2026 отдельным шагом с цифрами. **Процессное правило (принято):** behavior-влияющее изменение (может поменять выдаваемый код) — разрешение ДО, не объяснение после. Консолидация 4 матчеров — отклонена; развилка Portal/киоск — преждевременна (оба = виды CORE); только side-by-side одного документа безопасен.

## 2026-07-19 — Session: OTSKP каталог 2025_03 → 2026 (17 904 → 17 940), concrete-agent
**Rozhodnuto:** Alexander заметил, что доки говорят «17 904 OTSKP», хотя актуально 17 940. Разобрались: в репо ДВА committed XML — `2025_03_otskp.xml` (17904) и `2026_OTSKP_sfdi_import DZ.R4.Shared.xml` (17940), схема **идентична** (`<Polozka>`/`<znacka>`/`<nazev>`/`<jedn_cena>`/`<MJ>`/`<technicka_specifikace>`). otskp.db (pricing engine) уже строится из **GCS 2026** (`ingest_otskp_catalog.py` → `gs://…2026_OTSKP_sfdi_otevreny_format.xml`), а committed-XML потребители (`code_detector.py` прямой парс + `mcp/tools/otskp.py` in-memory fallback) висели на 2025_03/17904. Миграция (ветка `claude/otskp-2026-migration`): (1) `git mv` 2026-файл → чистое имя `2026_otskp.xml`; (2) репойнт `code_detector.py:37` + `mcp/tools/otskp.py:62,65` + `metadata.json` → 2026; (3) докстринги `otskp_engine.py`/`otskp.py` 17904→17940; (4) README ×3 → 17 940; `OTSKP_CATALOG_VERSION` дефолт уже был «OTSKP 2026». **Верифицировано реальным парсером локально:** parse-only 17940, полный parse→build_sqlite = 17940 строк + `catalog_version=OTSKP 2026` + реальная строка (015111 POPLATKY…), in-memory regex fallback тоже 17940. `test_catalog_embeddings.py` УЖЕ ждал 17940 (pgvector-путь мигрирован раньше) — миграция консистентна, ни один тест не завязан на 17904.
**Odmítnuto:** удаление старого `2025_03_otskp.xml` — оставлен как test-фикстура (`test_tubus_otskp_binding.py:316` читает путь, `test_otskp_ranking_golden.py` verbatim строки). find_otskp_code сигнатура/return не тронуты → MCP counter-файлы не меняются.
**Otevřené otázky:** URS_MATCHER имеет СВОЙ OTSKP (Node `otskpCatalogService.js`, 17904) + stavagent-portal свой (`import-otskp.js`) — отдельные копии, отдельная миграция на 2026 (их доки/стабы пока честно 17904). fastmcp локально не стоит → MCP-compat гоняет CI.
**Co dál:** довести `claude/otskp-2026-migration` до зелёного CI (test-mcp-compatibility) + merge; опц. мигрировать URS_MATCHER + portal OTSKP на 2026.

## 2026-07-19 — Session: URS_MATCHER аудит-ремедиация (#1526) + прямой frontoffice-каталог podminky.urs.cz
**Rozhodnuto:** (1) **PR #1526** (merge-commit `79b0496d`, ветка `claude/urs-matcher-audit-fixes`) — ремедиация аудита URS_MATCHER: reliability (WAL/busy_timeout/FK pragmas, non-fatal unhandledRejection, cache eviction), matching (diacritic folding case-preserving, token-overlap `max(lev,token)`, OTSKP cross-catalog guard, auto-learn только детерминированных локальных хитов, textNormalizer перестал стрипать числа), security (requireApiKey на harvest/prices/norms/smlouvy/workPackages, searchWeb opt-in, zip-bomb guard 25 MB, multer ^1→^2 + lockfile sync), **M7 `utils/jsonExtract.js`** (balanced-brace, заменил greedy `/\{[\s\S]*\}/` в 10 местах — «worked-then-stopped» на прозе Perplexity). CI-путь к зелёному: lockfile-sync fix + **revert удаления dead-code** (phase3Advanced.test.js импортит roleIntegration — рефакторинг вне scope, отложен) + фикс собственного golden-фикстура jsonExtract (control-assertion `toThrow()` был неверен: `[2]` — квадратная цитата, не ломает `{…}`; заменил на trailing `{pozn.}` prose-группу). (2) **Диагностика вскрыла настоящую причину** «ÚRS search не работает»: `/api/batch/diagnostics` → перплексити всё ещё `EMPTY_RESULT`. НЕ парсер (M7 нужен, но не достаточен) — а `SYSTEM_PROMPT_URS_SEARCH` ограничен `site:podminky.urs.cz`, а **podminky.urs.cz — это SPA**: голый fetch отдаёт JS-оболочку, `site:`-веб-поиск не индексирует динамику. (3) **Настоящий фикс (эта ветка `claude/urs-frontoffice-catalog-search`):** реверс-инжиниринг (Alexander снял из DevTools) **публичного JSON-API каталога** — `GET frontoffice-vqysm7dnza-ez.a.run.app/v1/search?versionId=<V>&query=<Q>&…` + `/v1/autocomplete`, **capability-URL** (без cookie/Bearer, скоуп только `versionId`). Новый `frontofficeClient.js` (`searchCatalog`/`autocomplete`, versionId в env `URS_FRONTOFFICE_VERSION_ID`=`dsdCAHQZh6lFvriEi3aB`/`CS_URS_2026_02`, fail-soft на []), встроен **первым** в `matchUrsItems` (детерминизм прежде Perplexity), confidence: exact code 1.0 / code_variant 0.9 / text 0.8 REFERENTIAL·0.75 COMMERCIAL (текст ниже 0.85 auto-learn gate — только точные коды кэшируются), kill-switch `URS_FRONTOFFICE_SEARCH=0`. Item-схема: `code/measureUnit/description/fullDescription/referenceCode/type(REFERENTIAL|COMMERCIAL)`, ответ `advancedSearch.items[]`. Frontoffice-проба добавлена в `/api/batch/diagnostics` (иначе юзер после деплоя видит только perplexity EMPTY и думает «не изменилось»). Golden-тест на реальном теле ответа (код 711113127 → 3 позиции).
**Odmítnuto:** удаление ~2350 LOK dead-code в #1526 (реверт — «рефакторинги не сейчас», ломало тест); metadata-авторезолв versionId (`/v1/version/metadata/<human>` — не знаю схему тела, не шиплю непроверяемый скан токенов — follow-up); Gemini→Vertex перевод LLM-слоя (отдельный PR по запросу — SPA не решает).
**Otevřené otázky:** точная схема `/v1/version/metadata` тела (для авторезолва versionId при смене релиза каталога); достаёт ли CORE `find_urs_code` до нового matcher-пути (в диагностике был `matcher:0`); из песочницы `*.run.app` и `podminky.urs.cz` заблокированы egress-политикой — живая проверка только у Alexandra/в проде/через MCP.
**Co dál:** ✅ #1527 merged (merge `81299575`) + **прод-verified** — ревизия `urs-matcher-service-00365-lw9`, `GET /api/batch/diagnostics` → `frontoffice: {status:OK, candidatesFound:3, timeMs:120, confidence:1, match_kind:exact_code}` (perplexity рядом остался EMPTY, ок — fallback). Follow-up (ветка `claude/urs-frontoffice-followups`): (1) **versionId авторезолв** из `/v1/version/metadata/<version>` — field-name-agnostic скан токена (16–32 base62, не человекочитаемая версия), кэш промиса, env-fallback `URS_FRONTOFFICE_VERSION_ID`; (2) **Gemini/Vertex routing видимость** в `/diagnostics` (`llmProviders.gemini.routing.activePath` = vertex_ai|api_key + project/location/model) — сам Vertex-путь в `llmClient.js` УЖЕ есть (пробует Vertex ADC первым, fallback на API-key), так что переключение = env-флип `VERTEX_PROJECT`+`GEMINI_MODEL` на Cloud Run, а не код. Осталось: Alexander флипает env на прод + опц. metadata-body для точного golden. **Gemini→Vertex (ветка `claude/urs-gemini-vertex-global`):** проверка на 19.07.2026 — `gemini-2.5-*` снимается **16.10.2026** (не ставить!); Gemini 3 линейка вышла, но `gemini-3-flash`/`gemini-3.1-pro` проекту недоступны (404 и в europe-west3, и в global), а **`gemini-3.5-flash` @ global → 200** (thinking-модель). Vertex-код бил только в региональный endpoint → добавлена global-ветка в `llmClient.js` (`VERTEX_LOCATION=global` → хост `aiplatform.googleapis.com` без регион-префикса; сконструированный URL 1:1 совпал с 200-пробингом) + `GEMINI_FALLBACK_MODEL` env (дефолт 2.5-flash landmine на 16.10). Флип: `GEMINI_MODEL=gemini-3.5-flash` + `VERTEX_LOCATION=global` + `VERTEX_PROJECT` + `GOOGLE_GENAI_USE_VERTEXAI=true`.

## 2026-07-16 — Session (d): element 24 Waves 3–5 (MCP + frontend + closeout) + хотфикс-пара #1516/#1517 закрыта

**Rozhodnuto:** (1) **Хотфикс-пара смерджена:** #1516 fail-close compute/AI на planner-api (service-key для MCP-делегата, SERVICE_API_KEY в cloudbuild atomically) + #1517 tz-ai-extract → канонический schema-validated force-JSON роут `POST /api/v1/tz/extract-calculator-fields` (манифест-промпт single-source в shared TS; typed errors `ai_invalid_json`/`llm_unavailable`). (2) **Wave 3 (MCP):** 3a forward тубус-полей в `calculate_concrete_works` (§2.10-guard: width_m-эвристика подавлена для тубуса); 3b тубус-soupis мимо breakdown-эвристик (honest NEPOČÍTÁNO вместо V/tl.; OTSKP bundling живёт в `breakdown.py::_attach_catalog_codes` `CATALOG_BUNDLING` — Q2 recon-факт); **3c §2.11 «Dotazy na projektanta»** — новый чистый детектор `dotazy_projektanta.py` (3 уровня: otazka/chyba/sloppy; Pattern 53 DO-пасмо = sloppy), `concrete_class_sources` в calculate → цитаты обоих источников с котвами + ПОЛНЫЙ вариант движка на каждый конфликтующий класс + CZK-delta из OTSKP DB (honest None при ненадёжной позиции); главный план всегда на явном вводе, движок никогда не выбирает; **3d prefab-план (AC2)** — `uzavreny_ram_tubus__prefab` шаблон (montáž ks + zálivka m³, количества только from_input), словарные коды `PRECAST.FRAME.INSTALL` + `CONCRETE.JOINT.GROUT` зарегистрированы covered (registration-proposal-as-PR; ратификация = ревью Alexandra), tally 13→15. (3) **Wave 4 (frontend):** tubus-блок step-3 (зеркало pilota-паттерна) — DC/габариты/толщины/технология A/B/auto/чекбоксы + превью геометрического объёма «kontrolní číslo, nikdy náhrada» (Q10); TubusCards в результате (фазы на DC, data-driven обоснование A/B + альтернатива, счётчик бетонажей 10×3=30, AC7-высота). (4) **Wave 5:** AC8 SKRUŽ vs STOJKY — data-правило `TUBUS_SUPPORT_RULE` (светлая ≤5 м [потолок одиночных стоек PROP_SYSTEMS] И нагрузка ≤50 kN/m²) с ДВУСТОРОННЕЙ живой калибровкой: Turnov 3,0/0,45 → 12,75 kN/m² → STOJKY (ложная скруж = класс ложного предупреждения ретро-листа), синтетика 6,5/0,8 → SKRUŽ со статическим посоузением; adversarial-свип нормализатора по 23 типам (30 фраз) → **отчёт-таблица в BACKLOG `normalizer-sweep-findings`**: 4 critical conf-0.9 (prostý-beton за tail-cut → фабрикация выztuže; «Zdivo nadzákladové» reject пробит substring'ом «základ»; двойной генитив «základů pilířů»→driky; wall-context не рулит типом) + 4 warning + 22 ok; **находки НЕ фиксились в PR1** (правило Alexandra: фиксы старых типов = отдельный PR после breakdown parity).

**Odmítnuto:** фикс свип-находок под флагом PR1 (Wave 5→9 никогда не закрылся бы); расширение `_TAIL_MARKERS` плошно (fix per механизм); UI-выбор варианта при конфликте классов (движок отдаёт обе, выбор за человеком).

**Otevřené otázky:** живая проверка кнопки AI-extrakce после #1517 (за Alexandrem — единственная незакрытая верификация пары); AC-строка «14/14» calculate_from_passport на полном паспорте — после деплоя.

**Co dál:** PR1 (Waves 1–5 одним PR, ветка `claude/stavagent-session-template-rm40yb`) — фокус ревью: 2 словарных кода (3d), AC8-пара тестов, шапка v2.1. Потом PR2 breakdown parity → фиксы свип-таблицы.

## 2026-07-16 — Session (c): SO 11-20-04 fixture — разбор подкладов, строковая верификация, golden в репо

**Rozhodnuto:** (1) `test-data/SO_11-20-04/` реорганизован по папкам `tz/` · `vykresy/` (13) · `vykaz/` (VV PDF + XML); мусорный файл `1` удалён; `XDC_ZM~1.XML` переименован в `XDC_ZM_1.XML` (имя, на которое ссылался golden). Добавлены `README.md` (инвентарь + карта «документ → секция golden» + honest gaps) и `SO_112004_tz_facts.md` (verbatim-цитаты с котвами «Strana X / 44» — конвенция SO-202). (2) **Строковая верификация golden против источников** (pdfplumber full-text TZ 62 стр., ElementTree XDC 88 объектов, VV PDF кросс-семпл): XDC — 45 позиций ✓, 38 OTSKP2025 + 1 OTSKP2026 + 6 R ✓, `mnozstvi==mnozstvi_puvodni`+нулевые dodatky ✓, все кубатуры jádra ✓; TZ — все параметры §2 (вкл. 3,0 vs 2,65), материалы §3, PB3-строка, DC-раскладка, HEB300/2×U300/2×U220, 14 дней, CPS I + B=600, Etapa I/II дословно ✓. (3) Golden положен в репо по конвенции: `test-data/tz/SO-11-20-04_podchod_golden_test.md` с правками: добавлена пропущенная позиция **87614 CHRÁNIČKY 5,100 M** (в XDC и VV есть, в golden не было); «44 stran» уточнено (44 нумерованных, PDF 62 вкл. приложения); §7 дополнен члением на 10 stavebních dílů TSKP 0–9; §1/§11 переведены на словарь task v2 (`uzavřený rám (tubus) / podchod / monolit`, полурám исключён); §5 Etapa II цитирует TZ честно («stěny u DC 2 a 4») с указателем на §9.2. (4) **Новая находка верификации — §9.3 расширен:** «základové desky a schodišť» приписано тубусовому DC без сходиште в ОБЕИХ этапах — golden знал только DC8 (Etapa I), строковая проверка нашла тот же класс у DC4 (Etapa II: «schodišť u dilatačních dílů 1, 3 a 4»). Пятый инцидент подряд: verification-пас ловит то, что ручной пас пропустил.

**Odmítnuto:** извлечение чисел из выкресов (vision-ось, Pattern 39 — отдельная работа; для golden хватает TZ+XDC); правка task v2 (загруженная v2_1 байт-идентична репо — канон уже на месте).
...
**Co dál:** Очередь пересобрана 2026-07-16 (risk-ranking по деньгам: ×2–×7 на опалубке при ÚRS/RTS бьёт живые приватные офферты раньше метаданных каталога): **PR1** тубус (task v2.1: §2.10 геометрия только из явных входов — запрет наследования breakdown-эвристик, привязка к pin #1514; §2.11 Dotazy na projektanta; AC 14–15) → **PR2** breakdown-geometry parity (выравнивание на TS-канон, снятие пинов #1514 фиксом) → **PR3** XDC-адаптер → **PR4** Fix 3+4 → **PR5** golden CI (блокирован выкресом выztuže). Pre-Implementation Interview PR1 проведён и адверсариально прогнан: 8/10 закрыто (Q1 один элемент с внутренними фазами; Q2 bundling-флаг как catalog-данные + факт-чек текущего enforcement; Q3 флаг «třídy z dokumentace», без флага = текущее поведение; Q5 палубные системы для стропа + PB-фильтр и на soffit; Q6 prefab через proposal-очередь словаря; Q7 §2.11-минимум со всеми тремя уровнями в схеме с первого дня; Q8 одна типовая délka; Q10 объём = выказ первичен, опалубка = только геометрия, расхождение → nález §2.11). Открыты два слова Alexandra: **Q4** категория пracnosti (per-фаза slabs_foundations/walls vs консервативный walls) и **Q9** классификационный дискриминатор «uzavřený vs otevřený průřez» (spodní deska решает, слова вторичны — SŽ зовёт подчоды «most»).

## 2026-07-15 — Session (b): parity-pin геометрии breakdown — documented divergence, третья система эвристик

**Rozhodnuto:** (1) Recon вскрыл постановку глубже задуманной: «TS-канона» у breakdown-геометрии ДВА (`element-geometry.ts` — dims→volume/fw, BACKLOG-канон; `estimateFormworkArea` в planner-orchestrator.ts — volume→fw эвристики, функциональный близнец Python-кода), а для двух осей (`curing_area` m², skruž obestavěný prostor m³) TS-контрагента нет вовсе. Диагноз (Alexander): breakdown построил ТРЕТЬЮ систему геометрических эвристик, не видя ни одного TS-источника — находка #11 была симптомом; числа `computed` вчерашнего конвейера детерминированы, но по формулам, которые TS уже чинил (детерминированный ≠ правильный). (2) Форма пина = **documented divergence** (не xfail): `tests/test_breakdown_geometry_parity.py`, 9 тестов, 3 класса — строки 1–4 (расхождение запинено с ОБЕИХ сторон: поведенческий пин Python + source-пин TS; одностороннее «выравнивание» = red → через разбор), строки 6–7 (пин односторонности: TS не содержит оси; появление контрагента = red), строка 9 (три набора толщин: 0.25/0.15 breakdown · 0.5/0.6 orchestrator · подтипная deck-таблица). Строка 11 = пин структурного потолка: `BreakdownElement` не несёт `width_m` → box-rule 2(L+W)·H непременим на MCP-поверхности в принципе; выравнивание строк 1–2 требует сначала контрактного изменения. (3) Вердикты по строкам: 1–4 TS прав; 5 (blinding) — доменный вердикт Alexander: дефолт = TS-ноль (подклад льётся в грунт), бортованное исключение = будущий opt-in флаг; 6–7 Python остаётся единственной реализацией. (4) **Живая коррекция recon-таблицы самим пином (метод работает):** строка 4 предсказывала soffit-ветку, а W3 `zaklady` помечен orientation=vertical (ось, которую TS флипнул в v4.17 C1!) → Python реально идёт стенной моделью с угаданной шириной 0.3: 200 m² vs TS-периметр ≈27.4 (~7×); orientation-дрейф добавлен в тикет констант.

**Odmítnuto:** xfail-форма (помечает боль, но не охраняет обе стороны); выравнивание формул в pin-ветке; rebar-константы в этом пине (ось классификатора → тикет); фикс blinding-флага здесь (только фиксация расхождения).

**Otevřené otázky:** три тикета `breakdown-geometry-divergence-followups` в BACKLOG: P1 `rebar-defaults-parity` (расширен: rebar 100↔120/130↔140/60↔Ø-градация + orientation drift zaklady); P2 `blinding-formwork-flag`; P2 `breakdown-element-width_m-contract` (разблокирует строки 1–2). Выравнивание строк 3/7 (толщина 0.25 → 0.5/подтипная таблица) — кандидат первой alignment-ветки.

**Co dál:** merge pin-ветки (divergence зафиксирован, не выровнен) → alignment-ветки по вердикт-таблице по одной строке за раз → затем развилка Bind-адаптер vs UWO F2.

## 2026-07-15 — Session: P1-гигиена — QuantityStatus: recon 3 словарей → 3 enum'а (дисциплина, не слияние)

**Rozhodnuto:** (1) Recon-first принёс карту: ключ `quantity_status` несёт ТРИ живые оси (join-outcome у elements · провенанс числа у items · полнота входа прайсинга у PricedPolozka — otskp_engine оказался ЖЕЛЕЗНИЧНЫМ движком, surface `/api/v1/soupis/generate`) + СПЯЩИЙ четвёртый диалект: колонка `quantity_status TEXT DEFAULT 'OK'` в Monolit `002-add-soupis-items.sql` без единого JS writer/reader. (2) Alexander ратифицировал: **три enum'а, не один** — в отличие от F3 (`bound`≡`candidate` были синонимами) здесь оси семантически разные; сливать не во что; унифицируется ДИСЦИПЛИНА (ни одной сырой строки, каждая ось названа). `ElementQuantityStatus` / `ItemQuantityStatus` / `PricingQuantityStatus` в `item_schemas.py` рядом с CodeStatus + **инвентарь-заголовок осей** в модуле (включая мёртвый диалект D и синонимический ряд `_source.volume_m3.status`) — анти-кладбище на уровне кода. (3) `NEPOČÍTÁNO(reason)` = prefix-константа + helper `nepocitano(reason)`; verbatim XLSX-контракт и `startswith`-тесты не тронуты. (4) Ветка чисто структурная: payloads байт-стабильны (call-site паттерн `.value` по прецеденту F3), goldens без изменений, guard `tests/test_quantity_status_enum.py` (7 тестов: канон значений, disjoint-оси, plain-str, source-inspection против сырых литералов, счётчик summary). CI-прогон: 85 (enum/quantify/join) + 91 (MCP-compat + passport goldens + stage-gating) + 139 (export/soupis/breakdown selection) — зелёные.

**Odmítnuto:** один merged-словарь (ложная унификация); подмешивание поведенческих фиксов в enum-ветку (a/b/c recon-находки) — зелёный CI должен ДОКАЗЫВАТЬ, что переименование ничего не сломало; parity-pin геометрии вторым коммитом (имеет право покраснеть → своя ветка).

**Otevřené otázky:** 4 тикета `quantity-status-recon-followups` в BACKLOG: P2 `collapsed_into_sibling` мимо summary-счётчика (поведенческий недосчёт); P3 фантомный `from_soupis` (решается при Bind); P3 `CHYBÍ_VSTUP` без продюсера (railway engine отдельно); P3 мёртвая Monolit-колонка (SQL не трогать).

**Co dál:** Ветка 2 — parity-pin формовочной геометрии breakdown (2×L×H, V/0.25, obvod×tl.) против TS `element-geometry.ts` по образцу pin B (`test_volume_geometry_parity.py` + CI-workflow). Затем развилка Bind-адаптер vs UWO F2 (см. entry 2026-07-14).
...
**Self-review #1510 (high, 8 úhlů → 10 nálezů) + fix-pack (kolo 4):** /code-review na merged Quantify-diffu našel, že kolo-3 fix sám vyrobil nový «sebevědomě-špatně»: (1) lineární větev 2×L×H stála PŘED orientation-checkem — horizontální prvek s L+H by dostal stěnovou formuli (mostovka 40 m² místo 2 420, s metkou computed); (2) ošetřování sdílelo qty_factor s bedněním — blinding-fix schlopl curing na boční pásek (154 m²) místo HORNÍHO povrchu (V/tl. ≈ 1 674 m²); (4) computed s defaultní tl. = maskovaný default. **Fix-pack (GO Alexander: top-4 + №10):** orientation rozhoduje PRVNÍ (horizontal nikdy nedostane stěnovou formuli); **vlastní `qty_factor='curing_area'`** s geometrií per orientation (horizontal = půdorys V/tl., vertical = líce bednění — hodnoty se kryjí, faktor oddělen, aby fix formovky nikdy netáhl curing); **pravidlo smíšeného provenance do SPEC §6.3**: computed jen když VŠECHNY činitele ze vstupu, jinak assumed; №5 pilota `length_m` = délka (from_input, dřív dropnuta za default 3.0 m); **№10 poslední míle**: `quantity_formula` → existující KROS sloupec «VV vzorec» (renderer už čte `vv_vzorec`), `quantity_status` → viditelný Zdroj-label («množ. assumed» křičí v XLSX, ne jen v JSON). Kolo 4 (SO-250 + SO-202 mostovka guard): blinding bednění 154.56 assumed / curing 1 674.4 (horní povrch); mostovka s L+H zůstává 2 420 assumed — žádný 2×L×H nesmysl. Testy: quantify suita 14→21, celkem 115 passed (1 fail = lokální bcrypt blocker, CI-covered). BACKLOG `quantify-review-1510-followups`: P1 QuantityStatus enum (TŘI slovníky pod jedním klíčem: join extracted/missing/ambiguous · breakdown from_input/computed/assumed/NEPOČÍTÁNO · otskp_engine OK/ODHADNUTO/CHYBÍ_VSTUP — precedent CodeStatus); P2 formwork-geometry parity-pin s TS enginem; P2 provenance single chokepoint (podmínka «tři místa» neplatila — interiér netknut).

**#1511 merged + nález №11 (session close):** Fix-pack shipnut jako PR #1511; CI chytil 2 zdroj-asserty (`test_calc_provenance_passthrough` — suita MIMO lokálně hnaný set assertovala PŘESNÝ Zdroj-label, №10 mu přidal «· množ. <status>»; intent testů — odlišitelnost NEPOČÍTÁNO řádků — zachován prefix-asserty). Poučení POTŘETÍ: hnát celý CI-set lokálně. Cestou přežit container-restart (lokální větev ztratila pushnutý commit → rebase na origin). **Nález №11 (Alexander nad tabulkou 2, verify-fáze ho propustila, slovník chytil): Skruž 2 420 m² vs slovník m³** — atom si protiřečil s vlastním vocabulary_code po jednotce; Gate-4 assertion kryl množiny kódů, NE jednotky. Follow-up commit: (1) **unit-parity assertion** v retrofit-suitě (MJ atomu == unit_canonical kódu, obě větve; sken potvrdil skruž jako jediný rozpor z 21 atomů); (2) **skruž → m³ obestavěného prostoru**: nový `qty_factor='falsework_volume'` = půdorys (V/tl. default) × výška pod NK (height_m) → `assumed`; bez výšky → řádek ZŮSTÁVÁ s quantity=None + `NEPOČÍTÁNO(chybí výška pod NK)` — žádná fabrikovaná výška (monolit větev poprvé nese None-řádky, skip-logika + catalog-binding None-guard = zavírá i latentní TypeError z nálezu №3); atomizer golden VĚDOMĚ změněn (m²·2420 → m³·None s komentářem proč). Poznámky bez akce potvrzené Alexandrem: dřík H_max ~+35 % zůstává v deferred thickness_m/variable-height; předpětí z TZ při plném SO-202 prohonu.

**Co dál (příští session, P1):** QuantityStatus enum + tři slovníky pod jedním klíčem (precedent CodeStatus) · parity-pin formovací geometrie s TS enginem. Fronta beze změny: MCP-transport stage-gating (BACKLOG P1), UWO F2, live pasport/composite verifikace.
...
## 2026-07-17/18 — Session: tubus UX-доводка + 8-угловое ревью + вердикты по большой таблице

**Rozhodnuto (merged):** #1521 tubus UX (скрыты нерелевантные генерические поля — Rozměry bloku [движковый гейт: тубус в NON_PRISMATIC_TYPES, box-объём для полой рамы не фабрикуется], Plocha bednění, Členění→заметка; правдивый Průměr výztuže; TZ-extraction тянет 7 полей §2.10 — regex+AI-манифест+apply; placeholders «např.») · #1522 fix-pack всех 10 находок 8-углового /code-review на #1521 (whitelist рамных слов против AC7-ловушки, mm→m, порядок «část+tl.+число», эллиптическое «spodní», sanity-диапазоны tubus_* в SANITY_RANGES = единый источник для AI-гейта и детерминистских emit-ов, кламп DC, честный §2.10-текст, MCP-докстринг, чистота). Shared 1483→1489.

**Klíčové pozorování (Alexander):** худшая находка ревью — «světlá výška pod podhledem 2,65 m» захватывалась как высота рамы с conf 1.0 — ровно ловушка 2,65-vs-3,0 из golden §2.2/AC7, немедленно переродившаяся в новом слое (экстракции). Подтверждение правила: **golden-ловушки живут поимённо — класс дефекта возвращается в каждом новом слое, пока его не запинишь в каждом.** Теперь запинена и в экстракции (adversarial-пины #1522).

**Recon велké tabulky (3 живых замечания, карты от 3 Explore-агентов):** №1 ручное označení beton не генерирует работы (генерация — только калькулятор; toggle меняет subtype лишь при m³-rep) · №2 две арифметики дней (замороженный schedule_info vs линейная сумма; 4 правила суммирования; 3 носителя созревания) · №3 экспорт «Jen monolity» пере-классифицирует исходный текст импорта, игнорируя текущий subtype.

**Verdikty Alexandra (2026-07-18):** GO №3→№1→№2 sequential · №3: единый предикат, одна функция+тест, ручное = истина · №1: вариант (а) — subtype='beton' всегда + путь через Vypočítat, второго генератора работ не будет; подслучай не-m³ rep = именной тест · №2: SDD-тикет с интервью, кода не писать; позиции: ручная правка = инвалидация (бейдж «ručně upraveno — přepočítat?», stale-флаг видимый), zrání предварительно = правка календаря, не трудочасов; «три носителя» — носитель один, остальные проекции · спека PR2 = docs/tasks/TASK_PR2_Breakdown_Geometry_Parity.md; порядок: №3 → №1 → recon PR2 → SDD №2 параллельно интервью.

**Co dál:** №3 реализован этим PR (isMonolithGroup, тикет monolit-jen-monolity-predicate) → после мержа №1 → PR2 recon. Turnov-прогон за Alexandrem (особо глазами: подсказка обязана взять 3,0, не 2,65).
