...
**Rozhodnuto:** Alexander: «правовой режим опускаем, важно реализовать технологию» → из пересмотренного порядка рецензии убран правовой шаг, реализован **фасад OTSKP** (reviewer #1, «одна дверь, не быстрая синхронизация копий»). Ветка `claude/facade-insulation-docs-analysis-3y8nba` была **stale** (23 коммита позади `origin/main`, её 8 ahead-коммитов = уже squash-merged #1526 URS-аудит) → сброшена на `origin/main` (Pattern 12 orphan, force-with-lease). На свежей базе: ядро на `2026_otskp.xml` (17 940) через #1530, но URS_MATCHER matching-дверь всё ещё парсила `2025_03_otskp.xml` (17 904) — один запрос через CORE и киоск давал разные коды. **Commit `c33a0bb9`:** новый единый источник `URS_MATCHER_SERVICE/backend/src/config/otskpCatalog.js` (`OTSKP_CATALOG_FILENAME`/`OTSKP_CATALOG_VERSION`/`OTSKP_KB_SUBPATH`/`OTSKP_DOCKER_XML_PATH`/expected 17940, всё env-overridable), 4 потребителя резолвят через него: `otskpCatalogService.js` (runtime, + `catalogVersion` в getStats/load-баннере), `import_otskp_to_sqlite.mjs`, `import_kros_urs.mjs`, `unifiedMatchingPipeline.js getCatalogInfo()` provenance-лейбл (= reviewer #5, версия в записи). Файл бандлится в контейнер киоска (Dockerfile.backend:43) → ядро и киоск читают ОДИН файл. Верифицировано локально: default резолвит в реально существующий 2026-файл обоими путями, env-override пинит назад в 2025 (клапан отката для незамеренного delta 17 904→17 940). MCP-контракты не тронуты.
**Odmítnuto:** правка Portal+Monolit — их живой OTSKP кормится НЕ ручным `import-otskp.js`, а `otskpAutoImport.js`/`migrations.js`/`routes/otskp.js`/Monolit `Dockerfile:20 COPY "…2025_03 OTSKP.xml"` (все на 2025, имя с пробелом, часть Render-era legacy). Перевод на 2026 требует физически положить 17 МБ XML в build-context каждого репо (Portal/Monolit НЕ бандлят KB ядра) — здесь Docker не собрать, XML не создать (deny-list). Правка одного ручного скрипта = раскол (ручной 2026 / runtime 2025) = та самая «быстрая синхронизация», которую reviewer отверг → откачена, оставлена как ОДИН infra-gated тикет (анкоры в handoff §5).
**Otevřené otázky:** корпус-50 (замер качества матчинга) всё ещё не снят — delta 17 904→17 940 по URS не измерен, откат = env-флаг; Portal/Monolit фасад ждёт provisioning 2026 XML в их build-context; из песочницы `*.run.app` заблокирован → живая проба у Alexandra/в проде.
**Co dál:** merge ветки; **корпус-50 = следующее действие** (снимает и delta OTSKP); затем Portal/Monolit фасад (infra-gated); затем side-by-side документ (Portal WF-B vs киоск document-extract). Консолидация фронтов и развилка — ПОСЛЕ CORE-оркестратора, не до.
