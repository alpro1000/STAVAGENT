...
**Otevřené otázky:** ратификация Этапа 1 (за Alexander) — на него теперь завязаны и frontoffice-query-стратегия, и диакритика-фикс; floor честного отказа (отдельная маленькая правка, go за Alexander); вердикты по флагам сметы — за Alexander; материалы: верификация требует другого источника (SPCM), вне текущего канала.
