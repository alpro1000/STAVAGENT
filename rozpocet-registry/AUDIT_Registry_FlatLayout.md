...
1. **Gap ×3 по interactive icons закрывается: 9 → 3** (§ 3.3). Строка содержит ровно 3 кнопки-иконки (checkbox, chevron, BarChart3), ровно по эталону Part A INFO row (chevron + Zap + Trash2, § 2.4.4). Количественный разрыв, зафиксированный как главный конфликт desktop-плоскости, устранён.
2. **Mobile row-level конфликты § 3.1 устранены** (вывод § 4.3.8). Ширина строки на 375 px помещается без horizontal scroll, checkbox всегда виден (sticky не нужен), hit-area chevron/checkbox достигает 44 × 44 px через `min-height` контейнера (паттерн Part A § 2.2), BulkActionsBar на mobile full-width без overflow (§ 4.3.6 media-query расширение). MoveUp/Down 11 px проблема hit-area исчезает — они переехали в detail panel.
3. **Прямое соответствие паттерну Part A § 2.5.2**. Planner поднимает действия вверх по иерархии (upload/export в toolbar, `Vypočítat` на element INFO row, удаление на element-уровень, work-row без per-row actions). Variant B делает дословно то же самое для Registry: skupina-level действия в toolbar skupiny, per-item контекстные — в detail panel, massive — в bulk bar. Row держит минимум.
4. **Принцип § 2.5.3 "сокращать, а не прятать" применён архитектурно, не косметически.** Variant A (Minimal) снижал визуальный вес стилем (shadows, padding), но оставлял 9 иконок. Variant B действительно **сокращает** количество row-level действий (9 → 3) через перенос на логически правильные слои, не через hover-reveal / overflow-menu / context-click. Overflow-menu исключён из рассмотрения per § 4.1.2 как отсутствующий в эталоне.
5. **Dilemma § 3.5 решена explicit для каждого двусмысленного элемента:**
   - `CircleHelp` / role-trigger → **detail panel** (путь B из § 3.5.3) — редкое ручное действие, требует полного контекста (kod, popis, rodič), в узкой action-зоне строки семантика иконки не считывается.
   - `BarChart3` / TOVButton → **остаётся inline как action** (путь B) согласно Domain rules (просмотр TOV — ключевая пользовательская задача киоска), **плюс** inline status-бейдж "▥ 45h · 180kg" (путь A для данных) рядом, чтобы разнести action и статус — паттерн § 3.5.2 "иконки Part A не совмещают status + action".
   - `HardHat` → **status badge inline** (путь A) + **action button в detail** (путь B) — чистый dual-role раздел, иконка в строке больше не clickable, navigate-to-Monolit живёт в detail где видим полный `mp.*` payload.
6. **Используются существующие компоненты, не создаётся параллельная структура**. `<BulkActionsBar>` (`BulkActionsBar.tsx:78-158`) расширяется (одна новая кнопка "Změnit roli" + media-query на mobile), `<GroupManager>` (`components/groups/GroupManager.tsx`) остаётся полной CRUD-панелью — в toolbar skupiny § 4.3.4 поднимаются только быстрые surface-действия (rename, delete, apply), не замещая. Undo/Redo stack (`useUndoStore`, `useUndoableActions`) получает один новый action-type `bulkSetRole`, по шаблону `bulkSetSkupinaUndoable`. Store-типы, backend sync, IndexedDB persistence не трогаются.

#### 4.3.10 Минусы

1. **Больший объём работы по сравнению с Variant A.** Variant A = ~9 CSS-токенов + ~8 size-атрибутов + 2 separator-спана (§ 4.2.2). Variant B требует: (a) новый `<DetailPanel>` компонент с layout, state-management и close-handler (~200–300 строк), (b) новый `<SkupinaToolbar>` компонент (~100 строк), (c) расширение `BulkActionsBar.tsx` с новым role-dropdown и mobile media-query (~50 строк), (d) новый store-метод `bulkSetRole` + undoable wrapper (~30 строк), (e) перестройка row-layout: удаление столбцов `actions` (70 px) и `monolit` из tbody (HardHat остаётся как badge в другой форме), добавление click-handler на `kod`/`popis` cells (~80 строк в `ItemsTable.tsx`). Итого ~8× объёма Variant A по коду, плюс значительно больший тестовый surface.
2. **Discoverability риск.** Пользователи, привыкшие к текущему Registry, искали MoveUp/Down/role-trigger/Link2 в строке — теперь их там нет. Первое взаимодействие на новом layout требует обнаружения, что "click на `kod`/`popis` открывает detail panel". **Mitigation**: CCL-пунктир (§ 4.3.7) под click-cells — единый визуальный сигнал "click here to edit / open", заимствованный из `<EditableNum>` паттерна Part A. Onboarding tooltip при первом входе в новую версию: "Klikněte na řádek pro podrobnosti". Risk всё равно остаётся — часть power-users в первый месяц будет фрустрирована отсутствием in-row action.
3. **Detail panel — новый UI-паттерн в Registry**, не существует в текущей кодовой базе. Требует explicit UX-решений по: (a) как именно открывается — inline row-expansion или side-drawer или bottom-sheet; (b) как закрывается — `[×]` button + Esc + click-outside + повторный click на той же строке; (c) keyboard navigation — tab-order через actions в панели, Enter на dropdown, Esc для закрытия; (d) accessibility — `<button aria-expanded>`, `role="region"`, `aria-label` на динамически-появляющемся блоке; (e) screen-reader announcements при открытии/закрытии; (f) single-open vs multi-open (в § 4.3.5 решено single-open, но это требует валидации). Каждое из этих решений — отдельная мини-спецификация.
4. **Document-navigation конфликты § 3.4 остаются out-of-scope.** Sheet tabs strip (`App.tsx:1187-1263`, whitespace-nowrap + counter → mobile horizontal-scroll конфликт, § 3.1), column filter dropdown в header (`ItemsTable.tsx:714-810`, stacking-context issue с BulkActionsBar z-50), ImportModal / PriceRequestPanel / MonolitCompareDrawer / AlertModal (§ 1.6) — всё это Variant B не адресует. "Плавающая папка + навигатор Stavba_Prodejna" из live-кейса пользователя не меняются. Фиксируется как отдельный issue для последующих сессий (см. `next-session.md` pending append в § 9).
5. **Риск неправильной классификации action'ов между слоями.** Варианты ошибок:
   - Если MoveUp/Down в реальности используются чаще, чем оценено в § 3.2.3 (где зафиксировано "rare, source Excel authoritative"), — их расположение в detail panel будет фрустрировать пользователей необходимостью каждый раз открывать detail для простой перестановки. **Mitigation**: классификация основана на § 1.2 Inventory + Domain rules + § 3.2.3 усиление source-order; финальная валидация — beta-период с метриками (click-rate на detail-reorder vs на inline-reorder в текущей версии).
   - Если `BarChart3` действительно не primary analysis action, а просто один из многих, — его сохранение inline (вместо миграции в detail) занимает лимит 3, который можно было бы заполнить чем-то более частым. **Mitigation**: Domain rules §1.1 CLAUDE.md прямо маркируют TOV workflow как ключевой для Kiosk BOQ Registry, поэтому решение обосновано не интуицией, а product-определением.
   - Если `Sparkles` + `Globe` в toolbar skupiny окажутся менее intuitive чем inline (пользователи искали per-row), их придётся мигрировать обратно. **Mitigation**: скрывать в § 4.3.4 toolbar только если user-testing подтвердит, что skupina-level ментальная модель совпадает с pattern'ом "применить эту категорию к похожим". Если не подтвердит — придётся держать в обоих местах (anti-DRY, но терпимо).
...
Три варианта сравниваются по пяти осям из § 4.1.4 (критерии, зафиксированные в intro Proposals). Каждая ячейка — factual вывод, не мнение; цифры и ссылки цитируют уже написанные подсекции 4.2 / 4.3 / 4.4.

| Ось | **Variant A — Minimal** | **Variant B — Lift** | **Variant C — Row-as-primary** |
|---|---|---|---|
| **1. Discoverability** | Максимальная: все 17 row-level элементов видны в тех же местах, где сейчас (§ 4.2.6 п. 3). Существующие пользователи не теряют ни одной точки входа. | Средняя: 3 inline icons + 4 click-cells с единым пунктирным CCL-сигналом (§ 4.3.7) + detail panel + toolbar skupiny (§ 4.3.10 п. 2). Один раз нужно изучить, что `kod`/`popis` click открывает detail, дальше single-pattern для всех edit. | Низкая: 3 inline icons + 0 click-cells — строка полностью read-only visually (§ 4.4.6 п. 4). Нет сигнала, что row clickable; пользователь открывает detail только через row click cursor-pointer hint, не через явный affordance. |
| **2. Скорость доступа к частому действию** (присвоение skupiny) | 1 клик на `<SkupinaAutocomplete>` в последней колонке **после** horizontal scroll на 375 px (§ 3.2.4). На desktop — 1 клик без scroll. | 1 клик на `<SkupinaAutocomplete>` cell inline с пунктирным affordance (§ 4.3.7). Scroll отсутствует на mobile (§ 4.3.8 — размещено в 375 px). | 3–4 клика: click на row → detail panel открывается → scroll к полю `Skupina` → click на autocomplete → select (§ 4.4.6 п. 1). Частая операция замедлена в ~3× относительно A и B. |
| **3. Визуальный шум** (icons + affordances в row) | **9 interactive icons** (§ 3.3 gap ×3 **не закрыт**) + 0 CCL + стилевые токены § 2.1–2.3. Визуально тише из-за убранных shadows/zebra/плотности (§ 4.2.6 п. 1), но количественный разрыв сохраняется (§ 4.2.7 п. 1). | **3 icons** (лимит § 2.4.4 соблюдён) + 4 CCL с пунктиром + 4 display (§ 4.3.9 п. 1, § 4.3.7 counts). Gap ×3 **закрыт**. | **3 icons** + 0 CCL + 8 display (§ 4.4.2). Наименьший визуальный вес — строка читается как plain data-линия (§ 4.4.5 п. 1, 4). Gap ×3 закрыт. |
| **4. Соответствие паттернам Part A** | Частично: визуальные токены § 2.1–2.3 цитируются дословно, но архитектура row (9 icons) **отходит** от § 2.4.4 (лимит 3) и от принципа § 2.5.3 "сокращать, а не прятать" (§ 4.2.7 п. 4). Partial +. | Полное: § 2.1–2.3 (токены) + § 2.4.4 (лимит 3 icons) + § 2.5.2 (lift + click-cell через `<EditableNum>` прецедент) + § 2.5.3 (принцип) + § 3.5.3 (путь A+B для dual-role #7) — все цитируются (§ 4.3.9 п. 3–5). Полный +. | Частично: § 2.4.4 (лимит 3) + § 2.5.2 (lift) + § 2.5.3 (принцип) respected, **но** click-cell паттерн § 2.5.2 (`<EditableNum>` в `FlatPositionsTable.tsx:893-963`) **явно отброшен** в пользу master-detail модели (§ 4.4.6 п. 3). Отход от прямого прецедента Part A. Partial +. |
| **5. Объём работы** | **S** — ~9 CSS-токенов + ~8 icon-size атрибутов + 2 separator-спана, 5 файлов правок (§ 4.2.2 total). Ничего нового не создаётся; store, undo, backend sync нетронуты (§ 4.2.6 п. 1). | **L** — новый `<DetailPanel>` (~200–300 строк), новый `<SkupinaToolbar>` (~100), расширение `BulkActionsBar.tsx` (~50), новый store-метод `bulkSetRole` (~30), перестройка row-layout в `ItemsTable.tsx` (~80). ~8× объёма A (§ 4.3.10 п. 1). | **L** — всё то же что B, плюс inline edit-контролы внутри detail (~100 строк форм для kod / popis / cena / skupina), **минус** удаление inline `<EditablePriceCell>` + inline `<SkupinaAutocomplete>` логики (экономия ~50 строк). Нетто ≈ как B, с перераспределением кода между row и detail. |

**Summary по осям.** Variant B выигрывает на 3 из 5 осей: скорость (1 клик vs 3–4 в C vs scroll-deпendent в A), визуальный шум (3 icons + 4 CCL paттерн Part A), соответствие Part A (5/5 принципов цитируются). Variant A выигрывает только на объёме работы (S vs L, S vs L) и на discoverability для **существующих** пользователей, но проигрывает по gap ×3 и по mobile-конфликтам § 3.1. Variant C выигрывает на визуальном шуме (минимум affordances) и чистоте "data-линии" строки, но платит 3× скоростью на частой операции и отходом от click-cell паттерна Part A — главный аргумент против на оси 4.

Trade-off между B и C сводится к одному выбору: **либо inline edit через паттерн click-cell § 2.5.2** (B — прямой прецедент Part A, быстрее для power-users, один CCL-сигнал к изучению), **либо clean read-only row + master-detail паттерн** (C — меньше шума, единый edit-UX, но отход от эталона + 3× замедление). Ось 5 (объём работы) не различает B и C; ось 1 (discoverability) — между B (нужен onboarding click-cell) и C (нужен onboarding row-click-opens-detail), оба требуют обучения.

---

### 5.2 Рекомендуемый вариант

#### 5.2.1 Выбор: Variant B — Lift

Рекомендация — **Variant B**. Обоснование выводится напрямую из матрицы § 5.1: B выигрывает 3 оси из 5 — скорость (1 клик до частой операции vs 3–4 в C vs scroll-зависимо в A), визуальный шум (3 icons + 4 CCL + 4 display, gap ×3 закрыт vs A где 9 icons сохраняются), и — ключевое для цели аудита — **полное соответствие паттернам Part A**: цитируются и реализуются § 2.1–2.3 (визуальные токены), § 2.4.4 (лимит 3 interactive icons в row), § 2.5.2 (подъём действий на слой выше + прямой прецедент `<EditableNum>` click-cell-to-edit из `components/flat/FlatPositionsTable.tsx:893-963`), § 2.5.3 (принцип "сокращать, а не прятать"), § 3.5.3 (explicit путь A+B для dual-role #7 HardHat). Цель аудита (привести Registry к стилю Part A) достигается только Variant B; два других варианта либо не закрывают структурный разрыв (A), либо отклоняются от прямого прецедента Part A (C).

#### 5.2.2 Почему не Variant A

Variant A **не рекомендуется как финальное решение**, потому что закрывает только косметический слой § 3 (стиль: shadows, alternating rows, hover color, иконки единого размера, padding 8/16 px — § 4.2.2), но **не закрывает структурный**: § 3.1 mobile overflow (~1 160 px ширина строки на 375 px viewport — § 4.2.5 "проблемы § 3.1 сохраняются"), § 3.3 gap ×3 по interactive icons (9 остаются в row — § 4.2.7 п. 1), § 3.5 dilemma статус-vs-action для `CircleHelp` и `BarChart3` (§ 4.2.7 п. 3 "откладывается"). Cost/benefit: S объём работы + S эффект. Variant A **может быть tactical Quick Win на 1–2 недели**, если развёртывание Variant B откладывается по причинам вне скоупа аудита (release calendar, загрузка команды), но как единственное финальное решение — оставляет главные конфликты неисправленными и формально противоречит принципу § 2.5.3, на котором построен весь audit.

#### 5.2.3 Почему не Variant C

Variant C **не рекомендуется**, потому что отходит от паттерна Part A `<EditableNum>` click-cell-to-edit, который в коде эталона **явно присутствует** (`components/flat/FlatPositionsTable.tsx:893-963` — 70 строк компонента с value+input совмещёнными в одной ячейке, click превращает display в edit) и был одной из трёх составляющих § 2.5.2 "что Planner делает вместо overflow". Variant C вручную отказывается от этого прецедента в пользу классического master-detail паттерна, которого в `components/flat/` нет ни в одном файле (§ 4.4.6 п. 3). Вторичный минус — discoverability edit-полей: строка в C выглядит полностью read-only (0 visual cues "тут можно редактировать" — § 4.4.6 п. 4), что противоречит основному пользовательскому сценарию киоска (быстрая классификация по skupinам через inline-взаимодействие). Единственное преимущество C над B по матрице § 5.1 — чуть более низкий визуальный шум (0 CCL affordances vs 4 пунктирных подчёркивания в B) — **не перевешивает** потерю прямого паттерна Part A, который и был целью всего аудита.

#### 5.2.4 Границы применимости рекомендации

- **Row-level конфликты § 3.1 закрываются полностью** (§ 4.3.8 итог): horizontal scroll исчезает (всё помещается в 375 px), checkbox всегда виден, hit-area 44 px через `min-height` контейнера (паттерн § 2.2), BulkActionsBar mobile full-width без overflow.
- **Document-navigation конфликты § 3.4 остаются out-of-scope** (§ 4.3.10 п. 4): sheet tabs strip (`App.tsx:1187-1263`), плавающая "корзина" и "папка" из live-кейса пользователя, ImportModal / PriceRequestPanel / MonolitCompareDrawer / AlertModal (§ 1.6) — адресуются отдельными issue в последующих сессиях.
- **Dilemma § 3.5 решена explicit** (§ 4.3.9 п. 5): `CircleHelp` / role-trigger → detail panel (путь B); `BarChart3` / TOV → inline как action + рядом inline status-бейдж (раздельные путь A для данных + путь B для действия); `HardHat` → inline status-бейдж + detail action button (dual-role).
- **Расширяемость**: архитектура Variant B позволяет добавлять новые display-cells и actions без структурных изменений. Пример — **каталожная цена OTSKP / ÚRS**: (a) новый display-cell `cena jedn. katalog` (рядом с `cena jedn.` и `celkem`) с цветовой индикацией перерасхода по паттерну `FlatPositionsTable.tsx:489-491` (`color: 'var(--red-500)'` при Výpočet > Katalog, `'var(--green-500)'` при ≤), (b) в detail panel — параллельное поле "Cena jedn. katalog: 3 150,00 Kč  · Odchylka: +2,9 %", (c) bulk-действие "Převést na katalogové ceny" в BulkActionsBar по шаблону существующего Tag → dropdown, (d) document-level "Aplikovat katalog na celý rozpočet" в toolbar документа по шаблону FlatToolbar'а. Четыре уровня (display в row + edit в detail + bulk + document) используют уже зафиксированные слои Variant B без добавления новых архитектурных паттернов.

---

### 5.3 Rollout

#### 5.3.1 Последовательность реализации

Variant B разбивается на **5 независимых PR**, каждый мержится самостоятельно. Логика порядка — от низкорискованного стилевого слоя к структурным изменениям; каждый следующий PR опирается на предыдущий, но не ломается при его откате.

**PR 1 — Flat style tokens** (§ 2.1–2.3)

Применить типографику, spacing, цветовую палитру, границы Part A к существующей таблице Registry. Дословный scope § 4.2.2: 9 CSS-токенов (`index.css`, `tokens.css`) + 8 size-атрибутов иконок в `ItemsTable.tsx` / `RowActionsCell.tsx` / `TOVButton.tsx` + 2 separator-спана. Убрать neomorph shadows (`--shadow-panel` `tokens.css:74-77`), убрать alternating rows (`index.css:135-137`), hover → `#F5F3F0`, uppercase 11 px headers, padding 8/16 px, icon sizes 11/13 px. **Без изменения структуры действий** — 17 row-level элементов остаются на своих местах. Этот PR == Variant A из § 4.2. Полезен сам по себе: даже если последующие PR откладываются, визуальная миграция улучшает соответствие Part A на токенном слое без регрессии.

**PR 2 — Toolbar skupiny** (§ 4.3.4)

Новый компонент `<SkupinaToolbar>` над каждым блоком položek одной skupiny (заменяет визуальную роль `rowRole === 'section'` строк из § 1.3). Поднять действия: `Sparkles` (`ItemsTable.tsx:865-874`), `Globe` (`:875-882`) из row-level; быстрый rename + delete surface из `<GroupManager>`; новый chevron collapse-all + info-бейджи `N položek` + `Σ Kč`. Удалить `Sparkles` и `Globe` из skupina-cell renderer'а в `ItemsTable.tsx`. Row-level счёт после этого PR: 9 → 7 interactive icons.

**PR 3 — Detail panel** (§ 4.3.5)

Новый компонент `<DetailPanel>` — inline expandable `<tr>` блок под активной строкой, открывается на click `kod` / `popis` cell, single-open семантика. Переместить внутрь: MoveUp/MoveDown handlers (`RowActionsCell.tsx:153-159`), role-trigger (`:180-205` + `updateItemRoleUndoable`), Link2 parent-picker (`:252-405`), HardHat navigate-to-Monolit action (`ItemsTable.tsx:530-548`, только action-half, status-badge остаётся inline), новый single-delete с opacity 0.4 паттерном § 2.4.5. Удалить column `actions` (70 px, `ItemsTable.tsx:458-471`) и HardHat click-handler из tbody. Row-level счёт после этого PR: 7 → 3 interactive icons — лимит § 2.4.4 достигнут.

**PR 4 — Extend BulkActionsBar** (§ 4.3.6)

Расширить `BulkActionsBar.tsx` новой кнопкой "Změnit roli" + dropdown (main / subordinate / section / unknown), handler `bulkSetRole(updates)` — новый store-метод по шаблону существующего `bulkSetSkupinaUndoable`. Добавить mobile media-query `@media (max-width: 640px)` — bar переключается на full-width bottom-sheet вместо centered pill (закрывает § 3.4.1 overflow на 375 px). **Не менять** геометрию на desktop (`fixed bottom-6 left-1/2 z-50` остаётся), не менять позицию / z-index — остальные geometric конфликты § 3.4.1 идут отдельным issue через `next-session.md`.

**PR 5 (опциональный) — Click-cells infrastructure** (§ 2.5.2)

Проверить, что click-cell edit работает на 4 CCL из § 4.3.2: `kod` (`:600-611`) и `popis` (`:614-625`) — нужно добавить click-to-edit inline text input (сейчас read-only); `EditablePriceCell` (`:30-76`) — **уже работает**, не трогать; `SkupinaAutocomplete` (`:820-863`) — **уже работает**, не трогать. Добавить общий CSS-класс для CCL-affordance (пунктирное подчёркивание из § 4.3.7) + hover-highlight + cursor: text. Финализирует паттерн § 2.5.2 `<EditableNum>` в Registry. Опциональность — PR 1–4 дают работающий Variant B даже без этого PR (kod/popis остаются read-only, но click открывает detail где их можно редактировать, и это fallback из Variant C).

#### 5.3.2 Testing gates

Для каждого PR — минимальный набор проверок в конце:

| PR | Что должно работать |
|---|---|
| **PR 1** | Визуальный diff vs pre-migration screenshot: desktop (viewport 1280 px) + mobile (375 px). Проверить: нет neomorph shadows на `.card`, нет zebra в `.table`, hover warm, padding ячейки 0 8px, первая ячейка 16 px. Store / undo / BulkActionsBar / filter dropdown / GroupManager — без регрессии (все handlers работают). |
| **PR 2** | `Sparkles` / `Globe` больше не рендерятся в skupina-cell строки (grep `ItemsTable.tsx` — 0 совпадений на эти icons в row-renderer). Toolbar над блоком položek показывает counter + sum + 4 action-button. `handleSparkles` / `handleGlobe` handler'ы вызываются из toolbar'а, работают как раньше. GroupManager CRUD-панель работает параллельно. |
| **PR 3** | Click на `kod` / `popis` cell → detail panel раскрывается inline под строкой. `[×]` / Esc / click другой row → закрывает. Single-open (максимум одна панель одновременно). Все 5 действий (MoveUp / MoveDown / role / Link2 / HardHat action) работают из detail. Column `actions` удалён из `<thead>`. Row-level счёт: 3 icons (checkbox + chevron + BarChart3). Keyboard nav: Tab → Enter → Esc работают. |
| **PR 4** | Selection 3 строк → BulkActionsBar показывает 5 кнопок (4 старые + "Změnit roli"). Новый dropdown "Změnit roli" открывается вверх, выбор роли применяется ко всем selected через `bulkSetRole`, undoable через Ctrl+Z. Mobile viewport 375 px: bar full-width, не overflow. |
| **PR 5** | Click на `kod` cell → inline input появляется в той же ячейке, `detail panel` **не открывается** (два взаимно-исключающих interaction-path на одной cell). Enter/blur сохраняет, Esc отменяет. Пунктир CCL visible on hover row. Проверить `popis` аналогично. `EditablePriceCell` / `SkupinaAutocomplete` — без регрессии. |

#### 5.3.3 Estimate

| PR | Сложность | Дни full-time (1 dev) |
|---|---|---|
| PR 1 | M | 2–3 |
| PR 2 | M | 2–3 |
| PR 3 | L (новый компонент, `<DetailPanel>` + click-handlers + close-logic + keyboard/a11y) | 4–6 |
| PR 4 | S (одна новая кнопка + один store-метод + media-query) | 1–2 |
| PR 5 | S если click-cell infrastructure расширяется из `<EditablePriceCell>` паттерна (~1–2 дня); M если строить с нуля общий `<EditableCell>` wrapper (~3 дня) | 1–3 |

**Итого**: **10–17 рабочих дней** на полную имплементацию Variant B при full-time работе одного разработчика. Нижняя граница — если PR 5 реюзает существующий EditablePriceCell паттерн; верхняя — если PR 3 требует дополнительной a11y / screen-reader итерации. Округлённо: **2–3 недели**.

#### 5.3.4 Rollback plan

Каждый PR независимый — если после PR N обнаружится регрессия, откатывается только этот PR без затрагивания предыдущих. Конкретные сценарии:

- **Откат PR 5** — click-cells на `kod` / `popis` возвращаются к read-only, edit идёт только через detail panel (это валидный Variant C-подобный fallback, не ломает UX).
- **Откат PR 4** — BulkActionsBar возвращается к 4 кнопкам, bulk role-change недоступен (users используют per-row role-trigger в detail panel).
- **Откат PR 3** — detail panel исчезает, row-level действия MoveUp / role / Link2 / HardHat navigate возвращаются в column `actions` и `monolit`. Это Variant A-подобное состояние с toolbar skupiny (PR 2) сверху.
- **Откат PR 2** — toolbar skupiny исчезает, `Sparkles` / `Globe` возвращаются в row-cell. Остаётся только PR 1 (flat style), что есть полный Variant A.
- **Откат PR 1** — стиль возвращается к neomorph. Нецелесообразен: PR 1 не меняет поведение, только визуальные токены — риск регрессии близок к нулю, откат не требуется ни при каких сценариях.

Порядок отката обратный порядку мержа: PR 5 → PR 4 → PR 3 → PR 2 → (PR 1). Каждый предыдущий уровень остаётся рабочим без последующего.

---

*Конец раздела 5. Out-of-scope находки — в `rozpocet-registry/next-session.md`.*
