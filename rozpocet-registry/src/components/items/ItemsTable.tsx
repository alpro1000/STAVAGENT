...
export function ItemsTable({
      ...
      ...(hasAnyMonolitData ? [columnHelper.display({
        ...
        cell: ({ row }) => {
          ...
          return (
            ...
            >
              <HardHat size={13} className={`${iconClass} w-[13px] h-[13px]`} />
            </button>
          );
        },
        size: 28,
  ...
  // Scroll container height is now governed by a flex-1 chain from <main>
  // down through the card, paired with max-height caps on the AI + group
  // manager panels (they scroll internally instead of pushing the table
  // below viewport). Earlier JS-driven maxHeight measurement removed.
      ...
      <div className="card flex flex-col flex-1 min-h-0">
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-border-color">
          <div className="flex items-center gap-2">
            ...
            >
              <Undo2 size={16} className="text-text-secondary w-[16px] h-[16px]" />
            ...
            >
              <Redo2 size={16} className="text-text-secondary w-[16px] h-[16px]" />
                      ...
                      {header.column.getIsSorted() && (
                        <span className="text-accent-primary">
                          {header.column.getIsSorted() === 'asc' ? (
                            <ChevronUp size={14} className="w-[14px] h-[14px]" />
                          ) : (
                            <ChevronDown size={14} className="w-[14px] h-[14px]" />
                          )}
                        </span>
                      )}
                    </div>
