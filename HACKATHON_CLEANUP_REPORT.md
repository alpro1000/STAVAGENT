...
- **`extracted_data/`** (~72 JSON+TXT files at root) — **FIXED in commit after initial review**: `git rm -r --cached` + added to `.gitignore`. Files remain locally and are regenerated by `extract_all_pdfs.py`; just no longer tracked in git.
