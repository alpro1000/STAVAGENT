...
- **Slug:** `peri_rcs_lite_climbing`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_rcs_lite_climbing/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI RCS LITE — šplhavé lešení
- **Title (en):** PERI RCS LITE — climbing scaffold
- **Vendor:** PERI
- **Product:** RCS LITE
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_rcs_lite_climbing` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_rcs_lite_climbing/source_navod.pdf .
```
