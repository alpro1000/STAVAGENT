...
- **Slug:** `peri_sb_brace_frame`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_sb_brace_frame/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI SB — opěrný rám pro jednostranné bednění
- **Title (en):** PERI SB — brace frame for single-sided formwork
- **Vendor:** PERI
- **Product:** SB
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_sb_brace_frame` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_sb_brace_frame/source_prospekt.pdf .
```
