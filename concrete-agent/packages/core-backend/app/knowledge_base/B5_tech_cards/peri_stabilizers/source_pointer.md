...
- **Slug:** `peri_stabilizers`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_stabilizers/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI stabilizátory — opěrné vzpěry bednění
- **Title (en):** PERI stabilizers — formwork bracing struts
- **Vendor:** PERI
- **Product:** Stabilizers
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_stabilizers` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_stabilizers/source_prospekt.pdf .
```
