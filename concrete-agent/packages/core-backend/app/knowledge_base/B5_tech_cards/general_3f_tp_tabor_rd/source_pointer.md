...
- **Slug:** `general_3f_tp_tabor_rd`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/general_3f_tp_tabor_rd/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** Technická zpráva — příloha II (rodinný dům, Tábor)
- **Title (en):** Technical report — annex II (single-family house, Tábor)
- **Doc type:** `project_tp`
- **Language:** cs
- **Files in folder (1):**
  - `source.pdf`
...
2. `git grep general_3f_tp_tabor_rd` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/general_3f_tp_tabor_rd/source.pdf .
```
