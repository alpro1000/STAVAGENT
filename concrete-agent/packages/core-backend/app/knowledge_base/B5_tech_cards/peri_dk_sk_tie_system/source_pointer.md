...
- **Slug:** `peri_dk_sk_tie_system`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_dk_sk_tie_system/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI DK / SK — technika spínání bednění
- **Title (en):** PERI DK / SK — formwork tie system
- **Vendor:** PERI
- **Product:** DK / SK
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_dk_sk_tie_system` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_dk_sk_tie_system/source_navod.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_dk_sk_tie_system/source_prospekt.pdf .
```
