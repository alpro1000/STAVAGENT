...
- **Slug:** `peri_variokit_vst`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_variokit_vst/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI VARIOKIT VST — bednicí vozík pro tunely
- **Title (en):** PERI VARIOKIT VST — tunnel formwork traveller
- **Vendor:** PERI
- **Product:** VARIOKIT VST
- **Doc type:** `tech_card`
- **Language:** cs
- **Files in folder (1):**
  - `source_navod.pdf`
...
2. `git grep peri_variokit_vst` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_variokit_vst/source_navod.pdf .
```
