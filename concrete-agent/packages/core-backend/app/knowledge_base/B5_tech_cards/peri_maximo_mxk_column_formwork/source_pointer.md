# peri_maximo_mxk_column_formwork
...
- **Slug:** `peri_maximo_mxk_column_formwork`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_maximo_mxk_column_formwork/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI MAXIMO MXK — bednění sloupů
- **Title (en):** PERI MAXIMO MXK — column formwork
- **Vendor:** PERI
- **Product:** MAXIMO MXK
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_maximo_mxk_column_formwork` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_maximo_mxk_column_formwork/source_navod.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_maximo_mxk_column_formwork/source_prospekt.pdf .
```
