...
- **Slug:** `peri_rcs_mp_loading_platform`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_rcs_mp_loading_platform/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI RCS MP — zásobovací plošina
- **Title (en):** PERI RCS MP — material loading platform
- **Vendor:** PERI
- **Product:** RCS MP
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_rcs_mp_loading_platform` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_rcs_mp_loading_platform/source_navod.pdf .
```
