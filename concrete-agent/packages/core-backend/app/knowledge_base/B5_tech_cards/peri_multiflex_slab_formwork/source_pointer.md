...
- **Slug:** `peri_multiflex_slab_formwork`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_multiflex_slab_formwork/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI MULTIFLEX — nosníkové stropní bednění
- **Title (en):** PERI MULTIFLEX — beam slab formwork
- **Vendor:** PERI
- **Product:** MULTIFLEX
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_multiflex_slab_formwork` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_multiflex_slab_formwork/source_navod.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_multiflex_slab_formwork/source_prospekt.pdf .
```
