...
- **Slug:** `peri_sky_anchor`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_sky_anchor/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI Sky kotva — kotvení šplhavého bednění
- **Title (en):** PERI Sky Anchor — climbing formwork anchor
- **Vendor:** PERI
- **Product:** Sky Anchor
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_sky_anchor` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_sky_anchor/source_navod.pdf .
```
