# walls_monolithic_cz_bba_monolit_tp01
...
- **Slug:** `walls_monolithic_cz_bba_monolit_tp01`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/walls_monolithic_cz_bba_monolit_tp01/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** Technologický předpis BBA-MONOLIT — monolitické stěny
- **Title (en):** Technological prescription BBA-MONOLIT — monolithic walls
- **Vendor:** BBA-MONOLIT
- **Doc type:** `tech_prescription`
- **Language:** cs
- **Files in folder (1):**
  - `source.pdf`
...
2. `git grep walls_monolithic_cz_bba_monolit_tp01` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/walls_monolithic_cz_bba_monolit_tp01/source.pdf .
```
