...
- **Slug:** `peri_srs_climbing`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_srs_climbing/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI SRS — šplhavé bednění s kolejnicí
- **Title (en):** PERI SRS — rail climbing system
- **Vendor:** PERI
- **Product:** SRS
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_srs_climbing` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_srs_climbing/source_navod.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_srs_climbing/source_prospekt.pdf .
```
