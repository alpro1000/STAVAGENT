...
- **Slug:** `peri_trio_wall_formwork`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_trio_wall_formwork/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI TRIO — rámové bednění stěn
- **Title (en):** PERI TRIO — frame wall formwork
- **Vendor:** PERI
- **Product:** TRIO
- **Doc type:** `tech_card`
- **Language:** cs
...
2. `git grep peri_trio_wall_formwork` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_trio_wall_formwork/source_navod.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_trio_wall_formwork/source_prospekt.pdf .
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_trio_wall_formwork/source_plakat.pdf .
```
