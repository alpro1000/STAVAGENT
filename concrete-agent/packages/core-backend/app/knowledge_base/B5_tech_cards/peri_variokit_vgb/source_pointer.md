...
- **Slug:** `peri_variokit_vgb`
- **Bucket:** `B5_tech_cards`
- **GCS path:** `gs://stavagent-cenik-norms/B5_tech_cards/peri_variokit_vgb/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** PERI VARIOKIT VGB — vozík pro římsy mostů
- **Title (en):** PERI VARIOKIT VGB — bridge cantilever forming traveller
- **Vendor:** PERI
- **Product:** VARIOKIT VGB
- **Doc type:** `tech_card`
- **Language:** cs
- **Files in folder (1):**
  - `source_navod.pdf`
...
2. `git grep peri_variokit_vgb` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B5_tech_cards/peri_variokit_vgb/source_navod.pdf .
```
