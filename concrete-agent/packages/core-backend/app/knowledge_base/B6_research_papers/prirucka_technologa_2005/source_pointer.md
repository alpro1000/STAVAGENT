...
- **Slug:** `prirucka_technologa_2005`
- **Bucket:** `B6_research_papers`
- **GCS path:** `gs://stavagent-cenik-norms/B6_research_papers/prirucka_technologa_2005/`
- **Vertex AI data store:** `urs-otskp-csn-norms-cenik` (region europe-west3)
- **Title (cs):** Příručka technologa (beton — návrh a výroba)
- **Title (en):** Concrete technologist handbook (mix design & production)
- **Doc type:** `handbook`
- **Language:** cs
- **Files in folder (1):**
  - `source.pdf`
...
2. `git grep prirucka_technologa_2005` finds the document, and
3. PR reviewers can see KB additions without reading the bucket.

## Fetching the source

```bash
gsutil cp gs://stavagent-cenik-norms/B6_research_papers/prirucka_technologa_2005/source.pdf .
```
