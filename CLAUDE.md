...
- [ ] **P1: MEGA pour Bug 4 — warnings severity refactor (deferred from v4.20)** — introduce parallel `warnings_structured: Array<{severity: 'critical'|'warning'|'info'; message: string; category?: string}>` alongside legacy `warnings: string[]`. Keep string[] in sync (derived from structured). Migrate existing "🚨 KRITICKÉ:" emoji-prefix messages → `severity: 'critical'` and reduce remaining pump duplication (the Bug 3 dedupe only handled pour-decision side — orchestrator still emits the MEGA zálivka warning separately). Update CalculatorResult.tsx:579 to render structured list with red/orange/blue palette instead of flat orange. ~50 push sites total; user agreed to parallel field approach, no breaking change to string[] callers.
