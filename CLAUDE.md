...
>
> **Changelog — v4.34.0 (2026-06-05 — TASK_2b: engine learns W3 element typing):** The UI-side TS engine now classifies structural element types as well as the W3 (MCP/Python) side, via a single source of truth + a ported head-noun algorithm. **Rule-DATA** extracted to `concrete-agent/.../app/classifiers/element_rules/element_types.yaml` (sibling of the January work-group `rules/`, a different axis — untouched), generated into the committed artifact `Monolit-Planner/shared/src/kb-generated/element-classification-rules.ts` by the EXISTING `scripts/gen-knowledge.mjs` (one new INTEGRATION entry + `yamlAbs` override; CI-blocking drift guard `gen:knowledge:check` extended to the `element_rules/**` path). The YAML carries `type_core` (coarse parity `family` + `bridge_boost` flag + `w3_name`), a `w3_family` directed roll-up (engine-fine → W3-coarse parity; generator enforces `family == w3_family[w3_name]`), `bridge_remap`, `object_type_aliases`, per-language `dictionaries.cs` (keywords/status/tail/bridge/wall/reject vocab; sk/de = future siblings, structure only). **Head-noun ALGORITHM** ported into the engine as a pure pre-layer `Monolit-Planner/shared/src/classifiers/element-name-normalizer.ts` (mirrors the W3 `app/mcp/tools/element_name_normalizer.py`): NK-beats-trám, context-`dřík` (bridge=pier / else wall), `základ` canonicalization, obklad→reject, dřík/opěra genitive suppression. **Selective** port — W3's unconditional `pilíř→pier` deliberately NOT ported (engine stays finer: building pilíř→sloup; bridge pilíř→driky_piliru via remap). `ClassificationContext` gains optional `construction_context` (bridge|retaining_wall|building), authoritative over `is_bridge` (additive back-compat; live callers still pass only `is_bridge`, so retaining_wall vs building is inert/test-only in 2b). Explicit **reject** (`ElementProfile.is_concrete_element=false` + `reject_reason`) for stone masonry cladding + special materials; `planElement` is the single authoritative site — zeroes rebar + all costs, warns "NEJSOU směrodatné". **Signal ladder**: OTSKP code = 1.0 (pinned); keyword ≤ 0.9 (conscious tier above fuzzy/AI, below code); genuine SAME-specificity tie → ≤0.7 + `candidates[]` (specificity guard: a more-specific keyword winning a generic sub-match is NOT ambiguous, e.g. "opěrná stěna" → clean operne_zdi). **dřík/opěra bug fixed once on BOTH runtimes** via the genitive-`opěr` qualifier (same logic as the `základ`-guard; `telo oper` removed from the pier keyword list). Parity asserted at **family level** (fine grain guarded by the engine suite, not W3). **1249 shared tests** (was 1088; +161, all W3-parity RED targets flipped green, zero control regression) + **19/19 W3 Python SO-250 goldens** (incl. new #77 dřík-opěra). Shipped as **PR #1308** (squash-merge — Pattern 12 orphan; content on main, branch commits orphaned). Convergence note in `docs/soul.md §9` lists the W3 goldens whose literal vocab flips at the eventual MCP classify-delegation (`operna_zed`→`operne_zdi`, `zaklady`→`zaklady_piliru`, `zdivo_obklad`→reject — all family-preserving; NO W3-bug assertions exist). Deferred: MCP classify-delegation · pre-compute reject UI badge · rich reject simple-volume · 3-way construction_context threading · sk/de dictionaries.
>
> **Changelog — v4.33.0 (2026-05-26 — Pattern library expansion: 20 new universal patterns 17-36 + 2 enriched):** Master registry `docs/STAVAGENT_PATTERNS.md` grew from 16 → 36 patterns (1727 lines, was 912). 20 new patterns extracted from the RD Jáchymov pilot CEV session (2026-05-18 → 2026-05-26) and reconciled into the master sequence: 17 Phase 0a Completeness Audit · 18 Iterative deepening with human-as-QA-gate · 19 Diminishing returns gate · 20 Audit v2 10-section completeness methodology · 21 Multi-factor catalog candidate selection · 22 PDF noise filters mandatory in matrix builders · 23 Per-drawing extraction beyond TZ-only · 24 Multi-namespace S-code/F-code handling · 25 Web search as catalog verification fallback · 26 Honest fallback hierarchy for missing data · 27 External LLM cross-validation as Nth source layer · 28 Schema integrity globally-unique entity IDs · 29 Continuous source provenance per item · 30 Czech regex diacritic boundary pitfall · 31 Comprehensive Extraction Verification (CEV) before catalog matching · 32 Two-file delivery audit + production separation · 33 Project synthesis before audit decisions · 34 Honest cost transparency to user · 35 Skill-of-the-pilot encoding for next iterations · 36 File staging convention for processed vs canonical inputs. **2 existing patterns enriched in-place** (numbers unchanged, content extended): Pattern 12 (Squash Merge Orphans) gained branch-lifecycle workflow + repo-settings algorithm from the Pattern S merge; Pattern 15 (Work-First, Catalog-Last) gained CEV pre-match consolidation gate (extends HK212's 3-stage workflow to strict 6-phase sequencing EXTRACT → CROSS-REFERENCE → CONSOLIDATE → VALIDATE LIST → MATCH CATALOG → PRODUCTION with explicit STOP gate after Phase 4 + frozen-snapshot artefact + Two-file principle) from the Pattern M merge. Master registry header bumped `last_number: 16 → 36`, `next_pattern: 17 → 37`, audit date 2026-05-26. Pilot-local case studies remain in their separate namespace at `concrete-agent/packages/core-backend/app/knowledge_base/B5_tech_cards/real_world_examples/rd_jachymov/patterns/01_*.md..09_*.md` (unchanged); a new sibling validation report at `…/rd_jachymov/patterns_validated.md` cross-references universal patterns 17-36 + enrichments to their pilot-local concrete artefacts (mirrors the Žihle convention). ZS_templates letter-namespace library (`B5_tech_cards/ZS_templates/PATTERNS.md` — Pattern A/B/C+, cost insights) stays untouched as a separate library. **Mandatory rules promoted to CLAUDE.md status** for the 5 highest-criticality patterns: **17 Phase 0a Completeness Audit** (pre-extraction gate, BLOCKS Phase 1 until completeness audit passes), **20 Audit v2** (10-section pre-delivery audit, sections E-J catch ~80 % of gaps), **31 CEV** (5 layers + 4 matrices before catalog matching, BLOCKS Phase 5 until Path A verdict), **15 Work-First Catalog-Last** (strict 6-phase sequence enforced, BLOCKS File B until File A frozen), **12 Squash Merge Orphans** (repo settings: disable squash globally; branches live long; PR only when truly done). Pattern 35 codifies the encoding discipline that produced this expansion. Sister concrete-agent CLAUDE.md updated with cross-references to new master patterns alongside existing pilot-local 01/05/08/09 mentions.
>
...
- **MCP Startup migrations** — `app/db/startup_migrations.py` runs SQL files at boot under session-level `pg_advisory_lock(8479_3162_5045_1287)` single-writer; `_open_connection(dsn)` with `connect_timeout=10` + 3-retry on transient OperationalError + `statement_timeout=30000`; `_resolve_dsn` strips `+asyncpg` and trailing whitespace from Secret Manager DSN; `_CRITICAL_SCHEMA` drift check on `mcp_api_keys`. Replaces dead cloudbuild psql step (Cloud Build VM has no Unix socket → silent no-op masked years of failures).
- **MCP CI** — `tests/test_mcp_compatibility.py` + `test_mcp_endpoints.py` + `test_mcp_auth_postgres.py` + `test_mcp_oauth_pkce.py` (~93 tests combined), `.github/workflows/test-mcp-compatibility.yml`. Runs on every push to concrete-agent/.
...
- **Element classification (v4.34, TASK_2b — learns W3):** rule-**DATA** single-sourced from `concrete-agent/.../element_rules/element_types.yaml` → generated `kb-generated/element-classification-rules.ts` (via `gen-knowledge.mjs`, drift-guarded). Head-noun **ALGORITHM** in `classifiers/element-name-normalizer.ts` (pre-layer: NK-beats-trám, context-`dřík`, `základ` canon, obklad→reject, dřík/opěra genitive suppression) — selective W3 port, `pilíř→pier` NOT ported (engine finer). `ClassificationContext.construction_context` (bridge|retaining_wall|building). Signal ladder: code=1.0 / keyword≤0.9 / same-specificity tie→≤0.7 + `candidates[]` (specificity guard). Explicit reject (`is_concrete_element=false`+`reject_reason`) → `planElement` zeroes rebar+costs. Parity at FAMILY level (`w3_family` roll-up). Fix once on both runtimes; W3 `element_name_normalizer.py` matched.
...
| classify_construction_element | `app/mcp/tools/classifier.py` + `element_name_normalizer.py` (W3) + shared KB `element_rules/element_types.yaml` (v4.34); engine parity in `Monolit-Planner/.../element-classifier.ts` |
...
- [ ] **P2: Physics validation rules batch — POST Part C, jeden task na Part B post-extraction cross-check infra (Patterns 51–55, docs-only zapsáno 2026-06-12):**
  - [ ] Pattern 51: pour-window feasibility hint — V ÷ finishing-governed rate vs ~20–24 h limit (KV 16 h → 1 takt ✓, Žalmanov 32 h → 3 etapy ✓)
  - [ ] Pattern 52: deck-thickness plausibility corridor — m³ ÷ plocha vs L/35–L/50 (dvoutrám předpjatý), gross-error trap only
  - [ ] Pattern 53: catalog DO-band ≠ project grade — «Z BET DO C40/50» je pásmo, C35/45 uvnitř ≠ nález (NotebookLM false alarm)
  - [ ] Pattern 54: TZ internal consistency — opakované parametry cross-check mezi sekcemi + geometrií, konflikt ⇒ flag s oběma citacemi (KV «5×20» vs «4×20»)
  - [ ] Pattern 55: mass-pour thermal protocol warning — CIRIA C660 → norms-bucket PŘED implementací pravidla
- [ ] **P2: Pattern 56 rotation economics (Fáze 3 crew/cost recon)** — rotace ⇒ nula §114 ZP přesčas; noční podíl 22:00–06:00 ⇒ noční příplatek cost-side; Nh invariantní k rotaci (verify proti v4.20 pour cost path)
- [ ] **P3: Pattern 57 «Rule 605» provenance discipline (ongoing)** — každé číslo etalonu/normy nese [TZ §X / VV pos. / CN / norma / odhad]; odhad nikdy tiše nepřebije tagged source
