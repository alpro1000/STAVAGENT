...
- **Calculator UX audit fixes (v4.17):** C1 zaklady_piliru orientation vertical→horizontal + estimateFormworkArea foundation-block special case. C2 maturity.ts CuringParams.exposure_class + EXPOSURE_MIN_CURING_DAYS (XF1=5d,XF3/XF4=7d,XD3=7d). C3 RECOMMENDED_EXPOSURE widened for zaklady/opery/operne/prechodova. C4 pile-engine.concrete_class echo+warning. C5 pile overpouring_m default 0.5. C6 getHeadsPerShift diameter-dependent table Ø600=5,Ø900=3,Ø1200=2,Ø1500=1.5. C7 CHA/PIT integrity tests opt-in. E3 lateral-pressure záběry are now MIN floor (num_tacts=MAX(volume,pressure)), not override-gated. A1 element_type change clears stale results. A2 DEFAULT_FORM.volume_m3=0, canCalculate gate, firstRun null when volume=0. A3 position status badge [Nový/Uložený/Upraveno] in header. B1 pilota formwork fields disabled-not-hidden with tooltip. E1 volume_mode manual/from_geometry, pilota locked to from_geometry. E2 L×W×H block for horizontal foundations (zaklady/patka/pas/opery) with auto volume+formwork area. D1 price_mode full/schedule_only toggle, KPI+cost table placeholder in schedule_only. D2 Pokročilé panel default open, Bednění section always visible (disabled for pilota). D3 Ceny section in sidebar with jeřáb/čerpadlo/vrtací souprava inputs.
...
- [x] **P1: Calculator UX walkthrough audit** — DONE: 24 bugs found via SO-202 TZ walkthrough (mostovka+opěry+pilíře+piloty). Golden test data saved to `test-data/tz/SO-202_D6_most_golden_test.md`. BUG-Z1/Z2/Z3/P1/P2/P3/P4 all FIXED in code. New bugs #11-#24 documented.
- [ ] **P0: Curing class 2/3/4** — `maturity.ts` CURING_DAYS_TABLE has only třída 2 values. Add `curing_class?: 2|3|4` param, expand table, orchestrator default 4 for NK/římsy, 3 for spodní stavba. Blocks production use on bridge NK (audit bug #1/#14).
- [ ] **P0: RECOMMENDED_EXPOSURE gaps** — Add XF4 to opery_ulozne_prahy, XF2 to driky_piliru. Currently false-positive warnings for standard SO-202 elements (audit bugs #11/#12).
- [ ] **P0: Wizard hint exposure** — useCalculator.ts:300-307 wizardHint2 calculateCuring call missing exposure_class param → shows 1.5d for XF2 (audit bug #2).
- [ ] **P1: Pile rebar default** — DEFAULTS.rebar_index_kg_m3 40→80 for bridge Ø900 (audit bug #13, 50% underestimate).
- [ ] **P1: Prestress days formula** — max(5, span/10) ignores cable count + injection time. SO-202 = 5d vs real ~11d (audit bug #7).
- [ ] **P1: Cross-kiosk sync Phase 3 remaining** — Portal 500 root cause (integration.js now returns structured errors but DB constraint causing 500 still unknown). Registry IndexedDB→PostgreSQL full rewrite (Cesta 2) deferred.
