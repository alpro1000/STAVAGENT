...
- `shared/src/calculators/lateral-pressure.test.ts` — **HIGH RISK — fragile coupling.** Sort stability MOCK_SYSTEMS using `formwork_category` (per sub-agent inventory F); pokud Gate 2 přidá `kategorie` axis nebo přejmenuje category, sort order může selhat. Sort stability v testech závisí na pořadí `formwork_category` enum hodnot. Při Gate 2 změnách v `formwork-systems.ts` enum je nutno spustit lateral-pressure testy izolovaně po každé změně. Doporučení pro Gate 2 task spec: explicit instrukce *„test isolation per enum change"* + možná konverze na deterministicky řazený seznam místo dependence on enum order.
