 ...
 *
 * NOTE: All handlers use async/await because db.prepare().all/get/run()
 * return Promises in PostgreSQL mode (production).
 */
...
router.post('/', async (req, res) => {
  try {
    ...
    // Sequential upserts — works for both SQLite (sync) and PostgreSQL (async)
    ...
    res.json({ success: true, rows: Array.isArray(savedRows) ? savedRows : [] });
  } catch (error) {
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    await db.prepare('DELETE FROM formwork_calculator WHERE id = ?').run(id);
