...
export const bridgesAPI = {
  ...
  create: async (params: { project_id?: string; bridge_id?: string; project_name?: string; object_name?: string; description?: string }): Promise<void> => {
    ...
    const portalAPI = (import.meta as any).env?.VITE_PORTAL_API_URL || 'https://stavagent-portal-backend-1086027517695.europe-west3.run.app';
...
export const uploadAPI = {
  ...
  getRegistryProjects: async () => {
    const { data } = await api.get('/api/import-from-registry/projects');
    return data;
  },
};
...
// Soupis prací moved to Portal (stavagent-portal/frontend/src/components/portal/DocumentAnalysis/)
