...
export default function FlatProjectSettings() {
  ...
  const handleWageChange = useCallback(async () => {
    ...
    const nonOverridden = positions.filter(p => p.wage_czk_ph === (config?.defaults?.DEFAULT_WAGE_CZK_PH ?? 398));
  ...
  const handleShiftChange = useCallback(async () => {
    ...
    const nonOverridden = positions.filter(p => p.shift_hours === (config?.defaults?.DEFAULT_SHIFT_HOURS ?? 10));
