...
interface Props {
  ...
  /** Whether the standard CONCRETE work types (bednění / výztuž / zrání /
   *  podpěrná konstr. / předpětí) may be added here. They belong to the beton
   *  calculator flow (auto-generated by «Aplikovat»), so a non-monolith group
   *  — a plain "jiné" line with no beton row and no calculator — offers only
   *  free-form «Vlastní práce». Defaults to true (monolith / beton groups). */
  allowConcreteWorks?: boolean;
  onClose: () => void;
}
...
export default function AddWorkModal({ partName, existingSubtypes, allowConcreteWorks = true, onClose }: Props) {
  ...
  const availableStandard = allowConcreteWorks
    ? STANDARD_WORKS.filter(w => !existingSubtypes.includes(w.subtype))
    : [];
  ...
  return (
    <div className="flat-modal-overlay" onClick={onClose}>
      <div className="flat-modal" onClick={e => e.stopPropagation()}>
        ...
        {!allowConcreteWorks && (
          <p style={{ fontSize: 12, color: 'var(--flat-text-label)', margin: '0 0 12px' }}>
            Betonářské práce (bednění, výztuž, zrání…) patří k monolitu a počítá
            je kalkulátor. Zde přidejte jen ostatní práce.
          </p>
        )}

        {/* Section 1: Standard works (concrete works — monolith groups only) */}
