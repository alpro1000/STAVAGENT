...
export interface PumpSpec {
  ...
  // Arrival pricing
  arrival_fixed?: number;    // CZK flat
  arrival_per_km?: number;   // CZK/km (×2 for round trip)
  // Operation pricing
  operation_per_h?: number;     // CZK/h
  operation_per_15min?: number; // CZK/15min
  volume_per_m3?: number;       // CZK/m³ (for hourly_plus_m3)
}

export type SurchargeModel = 'percentage' | 'per_hour' | 'flat';

export interface SupplierSurcharges {
  saturday?: { model: SurchargeModel; value: number };
  sunday?: { model: SurchargeModel; value: number };
  holiday?: { model: SurchargeModel; value: number }; // Falls back to sunday if not given
  night?: { model: SurchargeModel; value: number };
}
...
export interface SupplierData {
  ...
  pumps: PumpSpec[];
  hose_per_m_per_day?: number; // CZK per meter of extra hose per day
...
function applySurcharge(
  cfg: { model: SurchargeModel; value: number } | undefined,
  base_cost: number,
  hours: number,
): number {
  if (!cfg) return 0;
...
export function calculateSurcharges(
  ...
  if (surcharges) {
    ...
    if (dayType === 'holiday') {
      holiday_czk = applySurcharge(
        surcharges.holiday ?? surcharges.sunday, // fall back to sunday rate
        base_operation_cost,
        hours,
      );
    }
