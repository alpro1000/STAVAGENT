 ...
 * // ]
 */
export function scheduleProject(tasks, resources, calendar) {
    const schedule = [];
 ...
 */
function initializeResourceAvailability(availability, resources) {
    // Crews
 ...
 */
function getRequiredResources(task) {
    const resources = [];
 ...
 */
export function findCriticalPath(schedule, tasks) {
    ...
    for (const entry of schedule) {
        ...
        const successors = tasks.filter(t => t.predecessors && t.predecessors.includes(entry.task_id));
        let lateFinish = projectEnd;
        if (successors.length > 0) {
            // Late finish = min(late start of successors)
            lateFinish = Math.min(...successors.map(succ => {
 ...
 */
export function calculateProjectDuration(schedule) {
    if (schedule.length === 0)
        return 0;
    return Math.max(...schedule.map(s => s.end_day));
}
export function getScheduleSummary(schedule, tasks, resources) {
    const taskMap = new Map();
    for (const task of tasks) {
        taskMap.set(task.id, task);
    }
    ...
    const resource_utilization = new Map();
    return {
        total_duration_days,
        total_labor_hours,
        total_cost,
