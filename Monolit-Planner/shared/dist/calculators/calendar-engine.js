 ...
 */
export function isWorkingDay(date, config = {}) {
    ...
    if (!workingDays.includes(dow)) {
        ...
        if (config.saturday_half_day && dow === 6)
            return true;
        return false;
    }
 ...
 */
export function addWorkDays(startDate, workDays, config = {}) {
    ...
    while (counted < remaining) {
        if (isWorkingDay(current, config)) {
            ...
            if (holiday)
                holidaysInRange.push(holiday.name);
            if (config.custom_holidays) {
                for (const ch of config.custom_holidays) {
                    const chDate = typeof ch === 'string' ? parseDate(ch) : ch;
 ...
 */
export function countWorkDays(startDate, endDate, config = {}) {
    const start = normalizeDate(startDate);
 ...
 */
export function getMonthSummary(year, month, config = {}) {
 ...
 */
export function workDaysToCalendarDays(workDays, config = {}) {
    const workingDays = config.working_days ?? [1, 2, 3, 4, 5];
 ...
 */
export function calendarDaysToWorkDays(calendarDays, config = {}) {
    const workingDays = config.working_days ?? [1, 2, 3, 4, 5];
    const satHalf = config.saturday_half_day ? 0.5 : 0;
    const workDaysPerWeek = workingDays.length + satHalf;
