 ...
 */
export function getPourRateCoefficient(method) {
    switch (method) {
 ...
 */
export function parseMaxHeight(heights) {
    let max = 0;
    for (const h of heights) {
        const trimmed = h.trim().toLowerCase();
        if (trimmed === 'libovolná' || trimmed === 'libovolna')
            return Infinity;
 ...
 */
export function inferPourMethod(pump_typical, height_m) {
    if (height_m != null && height_m > 3)
        return 'pump';
    if (pump_typical)
        return 'pump';
    if (height_m != null && height_m <= 1)
        return 'direct';
    return 'crane_bucket';
}
