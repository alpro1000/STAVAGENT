...
// ─── Main Scheduler ─────────────────────────────────────────────────────────
export function scheduleElement(input) {
    const { num_tacts, num_sets: rawSets, assembly_days, rebar_days, concrete_days, curing_days, stripping_days, prestress_days = 0, num_formwork_crews = 1, num_rebar_crews = 1, rebar_lag_pct = 50, } = input;
    ...
    if (input.maturity_params) {
        ...
        const hours = getStripWaitHours(mp.concrete_class, mp.temperature_c, mp.element_type ?? 'slab', mp.cement_type ?? 'CEM_I');
    ...
    const nodes = [];
    const tactSetMap = new Map(); // tact index → set (0-based)
    // Per-tact duration accessors — fall back to uniform values when per-tact arrays not provided
    const getAssemblyDays = (t) => input.per_tact_assembly_days?.[t] ?? assembly_days;
    const getRebarDays = (t) => input.per_tact_rebar_days?.[t] ?? rebar_days;
    const getConcreteDays = (t) => input.per_tact_concrete_days?.[t] ?? concrete_days;
    ...
    const sched = new Map();
    const fwFree = new Array(num_formwork_crews).fill(0);
    const rbFree = new Array(num_rebar_crews).fill(0);
    ...
    while (remaining.size > 0) {
        ...
        for (const node of nodes) {
            ...
            if (node.ss_source) {
                ...
                if (!ss)
                    continue; // SS source not scheduled yet — skip
        ...
        if (!bestNode)
            throw new Error('Scheduling deadlock — cycle in DAG?');
        ...
        if (bestNode.crew === 'formwork' && bestCrew >= 0)
            fwFree[bestCrew] = finish;
        if (bestNode.crew === 'rebar' && bestCrew >= 0)
            rbFree[bestCrew] = finish;
    }
    // ─── 3. Compute results ──────────────────────────────────────────────────
    ...
    if (input.pert_params) {
        ...
        for (const nodeId of critical_path) {
            ...
            if (node.type === 'curing' && input.maturity_params) {
                ...
                const mp = input.maturity_params;
                const tp = curingThreePoint(mp.concrete_class, mp.element_type ?? 'slab', mp.temperature_c, mp.cement_type ?? 'CEM_I');
...
function computeCriticalPath(nodes, sched, total_days) {
    // Build FS successors map
    const fsSuccessors = new Map();
    for (const n of nodes) {
        for (const p of n.fs_preds) {
            if (!fsSuccessors.has(p))
                fsSuccessors.set(p, []);
    ...
    const ssSuccessors = new Map();
    for (const n of nodes) {
        if (n.ss_source) {
            if (!ssSuccessors.has(n.ss_source))
                ssSuccessors.set(n.ss_source, []);
    ...
    const sorted = [...nodes].sort((a, b) => sched.get(b.id).finish - sched.get(a.id).finish);
    const ls = new Map();
...
// ─── Utilization ────────────────────────────────────────────────────────────
function computeUtilization(nodes, sched, total_days, numFW, numRB, numSets) {
    let fwBusy = 0;
    let rbBusy = 0;
...
// ─── Bottleneck Analysis ────────────────────────────────────────────────────
function analyzeBottleneck(util, numSets, numFW, numRB, curingDays, workDays) {
...
// ─── Gantt Chart ────────────────────────────────────────────────────────────
function renderGantt(tacts, total_days, numSets, nodes, sched, numFW, numRB) {
