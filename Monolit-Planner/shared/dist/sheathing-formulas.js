 ...
 */
import { calculateCuring } from './calculators/maturity.js';
/**
 ...
 */
export function getCuringDays(concreteClass, temperatureCelsius = 20) {
    if (!concreteClass) {
    ...
    const validClasses = ['C12/15', 'C16/20', 'C20/25', 'C25/30', 'C30/37', 'C35/45', 'C40/50', 'C45/55', 'C50/60'];
    const matchedClass = validClasses.find(c => normalized.includes(c.split('/')[0])) || 'C30/37';
    const result = calculateCuring({
        concrete_class: matchedClass,
        temperature_c: temperatureCelsius,
    });
    return Math.ceil(result.min_curing_days);
}
 ...
 */
export function calculateAssemblyDays(area_m2, assembly_norm_ph_m2, crew_size = 4, shift_hours = 10) {
 ...
 */
export function calculateDisassemblyDays(assemblyDays, ratio = 0.5) {
 ...
 */
export function calculateSingleCycleDays(assemblyDays, curingDays, disassemblyDays) {
 ...
 */
export function calculateShiftDays(assemblyDays, curingDays, numKits) {
    if (numKits <= 1) {
    ...
    return Math.max(Math.ceil(optimalShift), assemblyDays);
}
/**
 ...
 */
export function calculateSequentialDuration(singleCycleDays, numKits) {
 ...
 */
export function calculateStaggredDuration(assemblyDays, curingDays, disassemblyDays, numKits) {
    if (numKits <= 1) {
        return {
 ...
 */
export function calculateAssemblyLaborHours(area_m2, assembly_norm_ph_m2) {
    return area_m2 * assembly_norm_ph_m2;
}
/**
 * Calculate labor hours for disassembly
 */
export function calculateDisassemblyLaborHours(assemblyLaborHours, ratio = 0.5) {
 ...
 */
export function calculateTotalRentalCost(numKits, dailyRentalPerKit, projectDurationDays) {
    return numKits * dailyRentalPerKit * projectDurationDays;
}
/**
 * Main calculation function - combines all formulas
 */
export function calculateSheathing(capture, config) {
    ...
    const assemblyDays = calculateAssemblyDays(capture.area_m2, assemblyNorm, crewSize, shiftHours);
    ...
    const singleCycleDays = calculateSingleCycleDays(assemblyDays, curingDays, disassemblyDays);
    // Sequential vs staggered
    const sequentialDuration = calculateSequentialDuration(singleCycleDays, numKits);
    const staggeredResult = calculateStaggredDuration(assemblyDays, curingDays, disassemblyDays, numKits);
    // Labor calculations
    const assemblyLaborHours = calculateAssemblyLaborHours(capture.area_m2, assemblyNorm);
    ...
    const timeSavingsPercent = sequentialDuration > 0
        ...
        : 0;
    // Summary text
    const summary = generateSummary(capture.part_name, capture.area_m2, assemblyDays, curingDays, disassemblyDays, numKits, workMethod, sequentialDuration, staggeredResult.duration, timeSavingsDays);
    return {
 ...
 */
export function generateSummary(partName, areaMm2, assemblyDays, curingDays, disassemblyDays, numKits, workMethod, sequentialDays, staggeredDays, timeSavingsDays) {
    const method = workMethod === 'staggered' ? 'шахматным методом' : 'последовательно';
    if (workMethod === 'sequential') {
        return (`${partName}: ${areaMm2.toFixed(1)} м² опалубки. ` +
 ...
 */
export function calculateAllCaptures(captures, config) {
    return captures.map(capture => calculateSheathing(capture, config));
}
/**
 * Calculate project-level statistics
 */
export function calculateProjectStats(results) {
    if (results.length === 0) {
        return {
            total_captures: 0,
            total_area_m2: 0,
