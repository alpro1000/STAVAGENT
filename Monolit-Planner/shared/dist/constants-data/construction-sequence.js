 ...
 */
export const BRIDGE_ELEMENT_ORDER = {
 ...
 */
export const BUILDING_ELEMENT_ORDER = {
    // Group 1: Foundations
    pilota: 10,
    podzemni_stena: 15,
    zakladova_patka: 20,
 ...
 */
export function getPartSequenceIndex(partName, sequence) {
    const upper = partName.toUpperCase().trim();
    for (let i = 0; i < sequence.length; i++) {
        if (upper.includes(sequence[i]) || sequence[i].includes(upper))
            return i;
    }
    return 999;
}
 ...
 */
export function detectProjectSequence(partNames) {
    const bridgeScore = partNames.reduce((s, n) => s + (getPartSequenceIndex(n, BRIDGE_PART_SEQUENCE) < 999 ? 1 : 0), 0);
    const buildingScore = partNames.reduce((s, n) => s + (getPartSequenceIndex(n, BUILDING_PART_SEQUENCE) < 999 ? 1 : 0), 0);
 ...
 */
export function sortPartsBySequence(partNames) {
    const sequence = detectProjectSequence(partNames);
    return [...partNames].sort((a, b) => getPartSequenceIndex(a, sequence) - getPartSequenceIndex(b, sequence));
}
